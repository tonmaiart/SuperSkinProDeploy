...
def _read_live_active_layer(storage, obj):
    ...
    a Weight Paint session is loaded AND at least one orphan bone is part of
    this session's mapping, or (None, None) otherwise.

    During the session, the active layer's true current weights live in
    ``__ssp_*`` temp VGs, ahead of ``ss_layer_N``. Reading this lets orphan
    detection reflect painting changes as they happen.

    Gated by ``_mapping_has_orphan_ids()`` first, so the full-mesh scan is
    skipped when there is no orphan to find. The scan is a single pass over
    the mesh (``read_active_layer_from_mesh()``) since this can run on every
    depsgraph tick while painting.
    """
...
class BoneIdentityService:
    ...
    def preview_orphan_weight(self, orphan_name: str):
        ...
        depending on whether a Weight Paint session is live:

        No session: composites *orphan_name*'s weight across all visible
        ...
        Weight Paint session: the orphan already has its own per-Layer temp
        VG (via get_unified_mapping()'s synthetic ID) holding the *active
        layer's* weight for it -- no new VG or weight write needed, just
        look its index up from ``__ssp_meta_map`` and point
