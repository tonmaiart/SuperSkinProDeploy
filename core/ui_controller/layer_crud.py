...
def _bake_and_reload_temp_vgs(ctrl, new_layer_index: int):
    ...
    try:
        ...
        if has_temp_vgs(obj):
            ...
            # Bake the multi-select pool (__ssp_pool) back into
            # storage.selected_names before _save_current_layer_state()
            # reads it a few lines down -- otherwise the OLD layer's META
            # would persist whatever pool existed at Enter Edit Mode instead
            # of the live pool at the moment this layer switch fired.
            storage_obj = obj.superskin_storage
            real_pool = read_pool_names(obj)
        ...
        # Restore the NEW layer's own pool from its META before seeding
        # __ssp_pool below, so the fresh temp-VG session starts from the
        # layer being switched TO, not the one being left. Safe to run here
        # (before the new temp VGs exist) -- it only needs obj.vertex_groups
        # (real VGs, already back after delete_temp_vgs() above), never
        # touches temp VGs itself.
        ctrl._restore_layer_state()

        full_pool = {n for n in obj.superskin_storage.selected_names.split(",") if n}
