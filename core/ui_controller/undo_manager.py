...
def _sync_after_undo():
    ...
    the Layer list widget itself, and invalidate the multi-select pool's
    draw-time read cache.
    ...
    try:
        ...
        if obj and obj.type == 'MESH' and obj.mode == 'EDIT':
            ...
            # Layer LIST refresh -- unconditional, NOT folded into
            # _sync_active_layer_from_meta()'s own sync_layers_to_ui_collection()
            # call, because that one only fires when the ACTIVE layer index
            # itself changed. Undoing a structural CRUD op (Add/Remove/
            # Duplicate/Merge/Move/Rename/Visibility -- now correctly BMesh-
            # undo-tracked via the __ssp_meta_list chunk layers) can revert
            # the layer LIST's contents without the active index moving at
            # all (e.g. undoing "Add Layer" removes the new entry but the
            # active pointer may already coincidentally match), which left
            # obj.superskin_layers_collection (what the N-panel actually
            # draws) stale until some unrelated redraw happened to touch it.
...
def _sync_active_bone_after_undo(obj):
    ...
    it repoints it at that bone's __ssp_L{layer_idx}_<bone_id> TEMP vertex
    group instead (so the built-in Vertex Group Weight overlay renders live
    ...
    *position*, not the real bone's own vg_index storage.last_clicked_index
    needs -- it has to be translated back through the naming convention
    (temp_vg_bridge.parse_layer_weight_vg_name()) via get_unified_mapping()'s
    id_to_bone table before it can be used to find the matching
    superskin_bones_collection row.
    ...
    if bone_name.startswith("__ssp_"):
        from ..layer_storage.temp_vg_bridge import parse_layer_weight_vg_name
        parsed = parse_layer_weight_vg_name(bone_name)
        if parsed is None:
            return  # a mask/meta/pool VG -- not a resolvable bone pick
        _, bone_id = parsed
