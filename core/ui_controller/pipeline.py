...
def flatten_to_mesh_edit(ctrl, dirty_verts: set = None, *,
    ...
    from ..layer_storage.temp_vg_bridge import (
        has_temp_vgs, has_layer_cache, read_temp_vgs_from_bm, effective_meta_list,
    )
    ...
    # Any Layer with its own resident temp-VG set this session (not just
    # the single active one) holds live, unbaked edits -- read each of
    # those from its own __ssp_L{idx}_* VGs instead of ss_layer_N/ss_mask_N,
    # so the compositor sees every cached Layer's in-progress state at once
    # (e.g. toggling visibility on a non-active cached Layer, or Merge
    # combining two already-edited-this-session Layers). Every other Layer
    # (never visited this session) still reads straight from permanent
    # storage, unchanged.
    session_active = has_temp_vgs(ctrl.obj)
    bm_active = bmesh.from_edit_mesh(mesh) if session_active else None

    for layer in meta:
        l_idx = layer["index"]

        if session_active and has_layer_cache(ctrl.obj, l_idx):
            if l_idx == active_idx and active_layer_override is not None:
                # Caller already has the complete, current active-layer dict
                # in memory (see docstring) -- skip the BMesh read entirely
                # instead of redundantly re-deriving the exact same data via
                # a per-vertex bv[deform] scan (a Blender-Python/C boundary
                # call, not a cheap dict lookup, so this scales with total
                # mesh vertex count on every tick regardless of brush size).
                ...
                # mesh.vertices during Edit Mode operations, so reading from
                # the BMesh directly guarantees that weight ops (which write
                # via write_layer_to_temp_vgs_bm) are visible here
                # immediately. Always a full read -- this becomes the
                # compositor's input for this Layer, which must represent
                # every vertex with real weight, not just the vertices this
                # tick's `dirty_verts` names (see docstring).
                layer_dict, mask_dict, _ = read_temp_vgs_from_bm(bm_active, ctrl.obj, layer_idx=l_idx)
            # Pass the dict straight through instead of encode()-ing it into
            # a pickled/zlib/base64 blob -- _composite_layers()'s harvest
            # loop decodes whatever lands in layer_data_map/mask_data_map
            # right back into a dict a few lines later regardless of source,
            # so encoding here was a pure round-trip that pickled+compressed
            # the ENTIRE layer every single tick just to immediately undo
            # it. LayerCompositor.composite_layers() (via codec.py) accepts
            # a raw dict value transparently alongside the encoded-string
            # entries the other (permanent-storage-backed) layers still use.
            layer_data_map[l_idx] = layer_dict
            if mask_dict:
                mask_data_map[l_idx] = mask_dict
        else:
...
def restore_layer_state(ctrl):
    ...
    # active_is_mask is intentionally left untouched here (no longer forced
    # to False on every layer switch) -- it is not itself part of the
    # per-layer META this function restores (only selected_names/locks/
    # active_bone_name are), so leaving it alone just means the current
    # Weight/Mask sub-mode carries over across a layer switch instead of
    # silently dropping back to weight editing. apply_active_bone() reads
    # this flag before any bone-name lookup, so it still takes precedence
    # over last_clicked_index below regardless of which one this function
    # touches. Removed per explicit user request (2026-09-08) -- matches
    # the same fix already applied to _enter_edit_mode() in
    # features/controller/ops_scene_modes.py for the same underlying bug
    # ("Edit Mask" button status getting silently reset), see that file's
    # comment and docs/core-interfaces/mode-edit-toggle-widget.md's
    # Guardrails section for the full rationale.
