...
def _log_compositor_loss(obj, meta, layer_data_map, result):
    ...
    if not ProfilerService.is_enabled():
        ...
        # unconditionally (see its own docstring: capture is always-on so
        # the Debug Console buffer stays complete regardless of which
        # category checkboxes are ticked, and SSPrefDebug's fields default
        # to True besides, so even reading the checkbox directly wouldn't
        # have gated this by default). Unlike the print()-only cost this
        # pattern caused elsewhere (see superskinpro-core-optimize skill
        # session notes), this function's body is a genuine diagnostic
        # computation -- decoding every visible layer's raw data plus a
        # nested per-bone total comparison -- so leaving it keyed to the
        # always-True is_enabled() meant it ran unconditionally on every
        # flatten_visible_layers_to_mesh() call: every Mirror, Auto Block
        # Weight, Clipboard paste, weight_transfer, and weight_apply
        # Object-Mode write, in production, for every user.
        return
...
def flatten_visible_layers_to_mesh(storage, obj):
    ...
    _write_result_via_bmesh(obj, result, num_verts)


_MIN_WRITTEN_WEIGHT = 0.001


def _write_result_via_bmesh(obj, result, num_verts):
    """Replace every real (non ``__ssp_``) deform weight with *result* in one
    bmesh round trip. ``VertexGroup.remove()`` costs the same on an empty
    group as on a populated one, so clearing hundreds of groups one by one
    dominated the whole flatten. Temp ``__ssp_`` entries are left untouched."""
    import array
    import bmesh
    ...
    for vg in obj.vertex_groups:
        if vg.name.startswith("__ssp_"):
            temp_idx.add(vg.index)
        else:
            name_to_idx[vg.name] = vg.index
    ...
    with profile_section("core.flatten.write_bmesh", len(result)):
        ...
        selection = array.array('b', bytes(num_verts))
        ...
        try:
            ...
            for bv in bm.verts:
                dvert = bv[deform]
                for group in [g for g in dvert.keys() if g not in temp_idx]:
                    del dvert[group]
