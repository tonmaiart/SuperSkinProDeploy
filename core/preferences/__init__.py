...
def _recover_orphaned_temp_vgs():
    """If a .blend was saved mid-Edit with temp VGs present, bake EVERY
    resident Layer's own temp-VG set back to permanent storage
    (ss_layer_N/ss_mask_N), not just the single active one -- a session can
    now have several Layers cached simultaneously (see
    core/layer_storage/temp_vg_bridge.py's per-Layer namespacing)."""
    try:
