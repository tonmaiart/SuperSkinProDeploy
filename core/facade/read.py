...
class ReadFacadeMixin:
    ...
    def get_meta_list(self) -> list:
        """Return layer metadata from the correct source for the CURRENT mode.

        Reads the in-session pending mirror (__ssp_meta_list) while a live
        Edit Mode session has one already written -- i.e. it reflects any
        layer add/remove/duplicate/merge/move/rename made earlier in the
        same Edit Mode session, before a real Exit Edit Mode flush. Falls
        back to permanent ss_layers_meta storage outside Edit Mode, or
        before any structural edit this session (see
        core/layer_storage/temp_vg_bridge.py's effective_meta_list(), the
        single source of truth this delegates to).
        """
        from ..layer_storage.temp_vg_bridge import effective_meta_list
        return effective_meta_list(self.obj, self.storage)
    ...
    def get_flattened_mask_dict(self, layer_index: int) -> dict:
        ...
        not just the active one. Always reads Object-Mode ``ss_mask_N``
        storage via ``storage.read_mask_dict()`` -- there is currently no
        caller that needs this from Edit Mode (the one caller as of this
        writing, overlay_color's Multi Color Mask Preview, only ever runs
        in Object/Pose Mode), so this does not check for or route through
        ``__ssp_*`` temp VGs the way ``get_active_mask_dict()`` does.
        """
    ...
    def get_selected_bones_pool(self) -> set:
        ...
        """
        from ..layer_storage.temp_vg_bridge import has_temp_vgs, read_full_pool
        storage = self.obj.superskin_storage
        if self.obj.mode == 'EDIT' and has_temp_vgs(self.obj):
            return read_full_pool(self.obj)
