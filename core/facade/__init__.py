...
# license system, advisory-only per docs/core-interfaces/license-gateway.md
...
class CoreFacade(ReadFacadeMixin, WriteFacadeMixin, VisualizerFacadeMixin):
    ...
    def active_layer_name(self) -> str:
        ...
        return self._layer_mgr.active_layer_name(
            effective_meta_list(self.obj, self.storage), self.active_layer_index
        )
    ...
    def suppress_meta_undo_push(self):
        """Context manager: suppress the automatic ``bpy.ops.ed.undo_push()``
        that ``set_active_bone_name()``/``apply_active_bone()`` (and other
        layer-meta writes routed through ``write_effective_meta_list()``)
        would otherwise push on every call — see
        ``core/layer_storage/temp_vg_bridge.py``'s ``suppress_meta_undo_push()``.

        Wrap a *passive* live preview (e.g. a modal's MOUSEMOVE hover
        handler) that calls those methods repeatedly just to drive a visual
        preview, not a real user commit — each call would otherwise push
        its own full undo checkpoint, which was the exact class of lag
        documented in ``docs/bug-history/0007`` (a cheap selection-only
        change routed through a mechanism designed for real data mutation).
        Do NOT wrap the real commit (e.g. on mouse release) — that call
        should still push its own checkpoint unsuppressed, or Ctrl+Z after
        the commit won't revert it.

        Usage::

            with ctrl.suppress_meta_undo_push():
                ctrl.set_active_bone_name(hovered_name)
                ctrl.apply_active_bone()
        """
        from ..layer_storage.temp_vg_bridge import suppress_meta_undo_push as _suppress
        return _suppress()
    ...
    def flush_edit_session(self):
        """Flush every resident Layer's in-session edits -- weights, mask,
        and structural metadata (Add/Remove/Duplicate/Merge/Move/Rename/
        Visibility) -- to permanent storage. Call exactly once, on a real
        Exit Edit Mode, while still in EDIT mode -- see
        core/ui_controller/layer_crud.py's flush_edit_session() for the
        full contract."""
        from ..ui_controller import layer_crud
        return layer_crud.flush_edit_session(self)
    ...
    def add_vg_selected(self, obj, name: str) -> bool:
        """Add *name* to the multi-select pool. Mode-aware: in Edit Mode
        with temp VGs present, toggles the __ssp_pool marker VG (undo-safe)
        for a real bone, falling back to selected_orphan_names for an
        orphan/out-of-range one. Outside Edit Mode, delegates to
        LayerCompositor unchanged."""
        from ..layer_storage.temp_vg_bridge import has_temp_vgs, toggle_full_pool
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            return toggle_full_pool(obj, obj.data, name, True)
    ...
    def remove_vg_selected(self, obj, name: str) -> bool:
        """Remove *name* from the multi-select pool. Mode-aware, mirrors
        add_vg_selected()'s branch."""
        from ..layer_storage.temp_vg_bridge import has_temp_vgs, toggle_full_pool
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            return toggle_full_pool(obj, obj.data, name, False)
        from ...core_subsystems.layer_compositor import LayerCompositor as _LC_sel
        return _LC_sel.remove_vg_selected(obj, name)

    def clear_all_selected(self, obj) -> None:
        """Reset the multi-select pool to empty. Mode-aware, mirrors
        add_vg_selected()'s branch."""
        from ..layer_storage.temp_vg_bridge import has_temp_vgs, write_full_pool
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            write_full_pool(obj, obj.data, set())
            return
        from ...core_subsystems.layer_compositor import LayerCompositor as _LC_sel
    ...
    def is_system_activated(cls) -> bool:
        ...
        of the license system (docs/core-interfaces/license-gateway.md),
    ...
    def get_layer_mask_state(cls, obj, layer_index: int) -> str:
        """Classify *layer_index*'s raw per-vertex mask coverage on *obj*
        into ``'WHITE'`` (every vertex fully masked-in), ``'BLACK'`` (every
        vertex fully masked-out), or ``'EDITED'`` (anything else -- a
        genuine, non-uniform per-vertex mask).

        Deliberately does NOT construct a CoreFacade instance -- unlike
        every read/write method above, *obj* need not be
        ``bpy.context.active_object``, no Pro-activation check runs, and
        the active Layer/mode is never touched. Safe to call once per
        Layer from a UIList ``draw_item()`` on every single redraw --
        added for the Layer list's per-row mask-state icon (see
        ``interface/utils/icons.py``'s ``get_layer_state_icon_id()``).
        *obj* must already carry a SuperSkinPro layer system
        (``"ss_layers_meta" in obj.data``); an *obj* with no layer system
        or no entry for *layer_index* reads as ``'EDITED'``.
        """
        from ..layer_storage.storage_service import LayerStorageService
    ...
    # ── Shared HUD Slot (classmethods — no instance required) ──────────
    ...
    def request_hud_slot(
        ...
        *,
        ...
        """Request a line on the shared bottom-left viewport HUD stack for *owner_id*.

        Use this instead of installing a feature-owned SpaceView3D draw
        handler for a status label -- two uncoordinated draw handlers
        writing to the same screen position is exactly the bug class this
        exists to prevent (see docs/bug-history/0030 for the analogous
        native-viewport-property version of this problem). Every active
        (non-expired) request draws on its own fixed row -- requesting a
        slot never hides another domain's already-active one, and an
        inactive slot leaves a gap rather than the rows above it sliding
        down to fill it.

        Deliberately a classmethod, not an instance method like
        ``show_toast`` -- callers that need this (watchers, cleanup/
        unregister paths, draw-adjacent toggles) cannot always guarantee an
        active mesh or Pro activation at the moment they need to
        request/release, unlike show_toast's call sites, which are always
        inside a live operator execute().
        ...
        Args:
            ...
            slot: Fixed row number, ``0`` at the anchor (bottom-most),
                increasing upward. A static reservation -- a row's screen
                position never shifts based on which other rows are active
                or when they were requested, unlike an earlier
                priority/requested_at ordering scheme this replaced. The
                canonical assignments (claim the next free integer here
                before using a new one):

                | Slot | Owner | Line |
                |---|---|---|
                | 0 | `deform_bone_viewer` | red "Edit Mask Weight" / aqua "Edit Bone Weight" |
                | 1 | `overlay_color` | pink "Multi Color" / gray "Single Color" |
                | 2 | `bone_picker` | yellow "BONE PICKER" |

                Two owners claiming the same slot is a caller bug, not
                something this call detects -- they simply draw on top of
                each other.
            timeout: Optional seconds after which this request auto-releases
                ...
                explicit release call site. None (default) holds the slot
                until release_hud_slot(owner_id) is called explicitly.
            color: RGBA color for this owner's text (e.g. distinguishing
                Multi Color Mode / Mask mode / Bone Picker at a glance).
            icon_texture: Optional pre-built ``gpu.types.GPUTexture`` drawn
                to the left of the text, tinted by *color*. Build one via
                ``interface/utils/icons.py``'s ``get_*_icon_texture()``
                helpers (e.g. ``get_layer_mask_icon_texture()``,
                ``get_bone_icon_texture()``). ``None`` (default) draws no
                icon.
        """
        from ..shaders.shader_manager import ShaderManager
    ...
    def release_hud_slot(cls, owner_id: str) -> None:
        """Release *owner_id*'s line from the shared HUD stack, if it currently holds one.

        Safe to call even if this owner doesn't currently hold a slot (no-op),
        and safe to call without an active mesh/context -- see request_hud_slot's
        docstring for why this is a classmethod.
        """
        from ..shaders.shader_manager import ShaderManager
    ...
    def clear_all_hud_slots(cls) -> None:
        ...
        Every current HUD-stack consumer (Multi Color Mode, Mask mode, Bone
        Picker) only makes sense while SuperSkinPro's own "Edit Layer Weight"
        mode is active, so call this from every Edit Layer Weight exit path
        (Save Weights, Force Pose Mode, the Tab/unguarded-exit auto-save
        guard) -- see docs/domains/controller.md's "Viewport State
        Invariant" section for why every exit path needs this, not just the
        explicit Save button. Safe to call with nothing active (no-op) and
    ...
    def get_dev_output_dir(cls, name: str) -> str:
        """Return a writable, addon-external directory for *name*, creating
        it if needed -- a per-user Blender data directory
        (``bpy.utils.user_resource('DATAFILES', ...)``), NOT a folder inside
        the addon's own installed tree.

        The sanctioned way for a dev-only export feature (profiler, debug
        console, support report, ...) to get a persistent output folder
        without knowing or computing the location itself. *name* must be a
        bare folder name (e.g. ``"debug_console_records"``), never a path.
        This intentionally does not live under the addon's own directory --
        see `DevRecordsService`'s docstring for why (Extension reinstalls
        wiping it, and addon-scoped file-access sandboxes being unable to
        read it back).
