...
class CoreFacade(ReadFacadeMixin, WriteFacadeMixin, VisualizerFacadeMixin):
    ...
    def add_vg_selected(self, obj, name: str) -> bool:
        ...
        from ..layer_storage.temp_vg_bridge import has_temp_vgs, pool_set_bone_bm
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            if pool_set_bone_bm(obj, obj.data, name, True):
                return True
            ...
            if name in names:
                return False
            names.add(name)
            storage.selected_orphan_names = f",{','.join(sorted(names))},"
            return True
    ...
    def remove_vg_selected(self, obj, name: str) -> bool:
        ...
        from ..layer_storage.temp_vg_bridge import has_temp_vgs, pool_set_bone_bm
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            if pool_set_bone_bm(obj, obj.data, name, False):
                return True
            storage = obj.superskin_storage
            names = {n for n in storage.selected_orphan_names.split(",") if n}
    ...
    def clear_all_selected(self, obj) -> None:
        ...
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            write_pool_names_bm(obj, obj.data, set())
            obj.superskin_storage.selected_orphan_names = ""
            return
        from ...core_subsystems.layer_compositor import LayerCompositor as _LC_sel
