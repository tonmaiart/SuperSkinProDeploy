...
# release-app doesn't otherwise touch the local working tree (it only reads
# blender_manifest.toml and dispatches a workflow against an already-pushed
# tag), but auto-committing/pushing here too means no release-* command
# leaves local work stranded -- see scripts/lib/git-sync.sh. Genuinely
# diverged history still hard-stops.
source "$REPO_ROOT/scripts/lib/git-sync.sh"
ensure_committed_and_pushed "$(git rev-parse --abbrev-ref HEAD)" "chore: auto-commit before release-app"
