...
# Auto-commits any uncommitted documents/ changes and pushes/fast-forwards
# the current branch against origin so a forgotten commit or push doesn't
# hard-block publishing -- see scripts/lib/git-sync.sh. Genuinely diverged
# history still hard-stops.
source "$REPO_ROOT/scripts/lib/git-sync.sh"
ensure_committed_and_pushed "$(git rev-parse --abbrev-ref HEAD)" "docs: auto-commit before release-docs" documents/
