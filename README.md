...
1. **Rust toolchain** — install via [rustup.rs](https://rustup.rs/). On
   ...
   workload via Visual Studio Build Tools. See `docs/core-interfaces/rust-logic.md`'s
...
2. **GitHub CLI (`gh`)** — lets you trigger and watch CI workflow runs from
   ...
   ```
3. **`git release-stage` / `git release-app` aliases** — one-time setup
   after cloning, since git aliases live in `.git/config` and aren't
   tracked by the repo:
   ```sh
   git config alias.release-stage '!bash "$(git rev-parse --show-toplevel)/scripts/release-stage.sh"'
   git config alias.release-app '!bash "$(git rev-parse --show-toplevel)/scripts/release-app.sh"'
   ```
   See `scripts/README.md` for what each does and its safety checks, and
   `docs/release-workflow.md` for the full staging → promote release flow
   these two implement.
...
| `git release-stage` | All 3 OSes, cleaned of dev-only files | Force-pushed to `tonmaiart/SuperSkinProDeploy` (**staging**, not production) |

`git release-app` (separate, manual, only after verifying the staged
build) then promotes that exact tested build to `tonmaiart/SuperSkinPro`
(**production** — what real users' in-app updater checks), with no rebuild.

See `docs/core-interfaces/rust-logic.md` for the full build-system writeup (why there
...
| Tab | Domains |
|---|---|
| `LAYER` | `layer_viewer` (non-collapsible), `tool_socket` (dropdown slot; currently hosts `weight_transfer`, which owns Export/Import JSON too — see `docs/domains/tool_socket.md`) |
| `SKINNING` | `deform_bone_viewer` (non-collapsible), `weight_apply`, `mirror`, `controller`, `tool_socket` (dropdown slot; currently hosts `clipboard`, `in_mesh_transfer`, and `auto_block_weight`) |
