...
class ProfilerService:
    ...
    def export_to_file(cls) -> str:
        ...
        Written to ``profile_<timestamp>.json`` under
        ``DevRecordsService.get_dir("profiler_records")`` -- a per-user
        Blender data directory, not inside the addon's own installed
        folder -- creating that folder on first use. Includes the
