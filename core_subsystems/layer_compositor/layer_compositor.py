...
class LayerCompositor:
    ...
    def remove_layer(cls, meta_list, index):
        """Remove a layer by slot *index*. No minimum is enforced -- this can
        return an empty list when *index* was the last remaining layer."""
    ...
    def get_layer_cut_mask(cls, meta_list, mask_dicts_map, target_index, num_verts) -> dict:
        """Return ``{v_idx: float}`` -- *target_index*'s own mask, "cut" down
        to only the fraction that survives the real compositor's top-down
        alpha-blend against every VISIBLE layer stacked above it, mirroring
        `find_mask_gaps()`'s same `opacity_leak *= (1.0 - v_mask)`
        accumulation, just restricted to the layers above *target_index*
        and then multiplied by its own mask.

        **Ordering.** *meta_list* is display order (top-to-bottom, as
        `read_meta_list()` returns it) -- NOT the bottom-to-top order
        `codec._composite_layers()` reverses it into before calling Rust
        (see that function's `ordered_meta = reversed(meta_list)`). A layer
        "stacked above" *target_index* is therefore any visible layer that
        appears BEFORE it in *meta_list*'s own (unreversed) order -- the
        loop below simply walks *meta_list* until it hits *target_index*
        and stops.

        **Known approximation vs. the real Rust engine
        (`layer_compositor.rs::composite_layers_engine`).** This only
        accumulates layers' MASK values -- it does not replicate the real
        engine's `is_base`/"layer has no weight data at this vertex"
        gating (a layer with a nonzero mask but literally no weight
        painted anywhere at a vertex contributes no cut there in the real
        engine, but does here). Same simplification `find_mask_gaps()`
        above already ships with for the identical reason: a mask-only
        preview does not have that layer's weight data to check in the
        first place, and this is a UI visualization aid, not a source of
        truth for the actual deform result. See
        docs/domains/overlay_color.md's "Isolate mode" note for the
        feature this backs.

        Returns only vertices whose cut value exceeds ``0.001`` (mirrors
        the Rust engine's own epsilon thresholds) -- a sparse dict, same
        shape convention as `get_active_mask_dict()`.
        """
        ...
        own_mask = mask_dicts_map.get(target_index, {})
        own_default = float(target_meta.get("mask_default", 1.0))
        result = {}
        for v_idx in range(num_verts):
            v_mask = own_mask.get(v_idx, own_default)
            v_mask = max(0.0, min(1.0, v_mask))
            cut = v_mask * leak[v_idx]
            if cut > 0.001:
                result[v_idx] = cut
        return result
