...
feature domain -- see ``docs/domains/debug_console.md``) no longer gate
...
class DebugLogService:
    ...
    def log(category: str, message: str) -> None:
        """Buffer *message* under its category and mirror it to the system console only when console printing is on.

        The buffer is capped at ``_MAX_LOG_LINES`` (oldest dropped first) and feeds
        the debug console's live log view.
