...
def _register_debug_log_callback(module) -> None:
    """Hand the compiled Rust core a live ``DebugLogService.log`` callback so
    its internal timing/diagnostic messages (e.g. the license-vs-compute
    breakdown in ``rust_composite_layers``/``_mixed``/``rust_smooth_logic``)
    route into SuperSkinPro's own Debug Console (``rust_ffi`` category)
    instead of raw ``eprintln!``/stderr, which only a System Console or a
    process-level debugger can see.

    Lazily imported (not at module scope) for the same F3 Reload Scripts
    ordering reason ``PreferencesService`` is imported lazily in
    ``RustWeightEngine.__init__`` below: ``core_subsystems/__init__.py``'s
    reload loop reloads this package before ``debug_logging``, so a
    module-level import here would permanently bind a pre-reload
    ``DebugLogService`` class object.

    Guarded by ``hasattr()`` since an older ``rust_logic`` binary built
    before this bridge existed won't export
    ``rust_register_debug_log_callback`` -- it simply keeps using its
    original ``eprintln!``/stderr output in that case.

    Called from ``_internal_load_binary()``, i.e. once per that function's
    own memoisation cycle (once per F3 Reload Scripts, not once per FFI
    call) -- so the callback always points at the *current*
    ``DebugLogService`` class after every reload, even though the compiled
    binary itself never reloads.
    """
