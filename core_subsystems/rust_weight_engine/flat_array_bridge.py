...
# =========================================================================
#  Int-keyed layer-dict <-> COO (hot-path calculators: add/scale/smooth/sharpen)
# =========================================================================

def int_layer_to_coo(
    layer_int: dict,
    *,
    dtype_vert_ids: str = "q",  # signed long long (i64) -- matches the
                                # i64 vertex-index type these calculators use
    dtype_bone_ids: str = "i",  # signed int (i32)
    dtype_weights: str = "d",   # double (f64)
) -> Tuple[array.array, array.array, array.array]:
    """Convert ``{v_idx: {bone_id: weight}}`` to flat COO arrays, for layers
    where ``bone_id`` is ALREADY the final int vertex-group index -- unlike
    ``layer_to_coo_sparse()`` above (which maps bone NAMES through a
    caller-supplied ``bone_to_id``), this needs no remap step, since the
    hot-path calculators (add/scale/smooth/sharpen) already receive
    int-bone-ID layer dicts from `CoreFacade`'s unified mapping, never
    string bone names.

    Args:
        layer_int: ``{v_idx: {bone_id: weight, ...}, ...}``.

    Returns:
        ``(vert_ids, bone_ids, weights)`` as parallel ``array.array``
        objects, all the same length (total vertex-bone weight entries).
    """
    total_entries = sum(len(w) for w in layer_int.values())

    # A zero-length array.array has no backing allocation, and PyO3's
    # PyBuffer<T> extraction on the Rust side rejects that buffer's pointer
    # as insufficiently aligned -- pad with one inert row instead so the
    # buffer crossing the FFI boundary always has a real allocation.
    # `coo_to_int_layer()` strips this row back out on the way back.
...
def coo_to_int_layer(
    ...
    """Inverse of ``int_layer_to_coo()`` -- rebuilds ``{v_idx: {bone_id:
    weight}}`` from parallel COO arrays returned by a hot-path calculator."""
    result: dict = {}
    for v, b, w in zip(vert_ids, bone_ids, weights):
        v_int = int(v)
        if v_int < 0:
            # `int_layer_to_coo()`'s empty-input padding row, unchanged by
            # the Rust calculator since no real vertex has a negative index.
            continue
