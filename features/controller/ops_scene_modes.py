...
from ...core.layer_storage.temp_vg_bridge import (
    ...
    read_temp_vgs_from_bm, read_pool_names_from_bm,
)
...
def _enter_edit_mode(op, context):
    ...
    # Every fresh entry into Edit Layer Weight defaults to bone editing, not
    # Mask editing. active_is_mask now persists across Save (the Deform
    # Bones list's "Mask" toggle, features/deform_bone_viewer/ops.py,
    # deliberately preserves it so Save doesn't clobber the list's bone
    # selection) -- without this reset, saving out while Mask editing was
    # on would silently resume straight back into Mask mode on the next
    # Enter Edit Mode instead of the last-active bone.
    ...
    # Always False at this point -- reset just above, right after
    # `obj = target_mesh` -- so every fresh Enter Edit Mode starts on the
    # 'SINGLE' (bone) visualizer, never 'MASK'.
...
def _bake_temp_vgs_on_exit(obj):
    ...
    try:
        ...
        # Bake the multi-select pool (__ssp_pool, undo-safe during the
        # session -- see docs/bug-history's follow-up to 0032) back into
        # storage.selected_names before it's read below. Real bones come
        # from the BMesh-tracked pool VG; orphan bones (no real VG, can't
        # use the __ssp_pool slot trick) come from the parallel string
        # property, which never had a live temp-VG mirror during the
        # session in the first place.
        storage_obj = obj.superskin_storage
        real_pool = read_pool_names_from_bm(bm, obj)
        orphan_pool = {n for n in storage_obj.selected_orphan_names.split(",") if n}
        merged_pool = real_pool | orphan_pool
...
class SUPERSKIN_OT_enter_layer_edit(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        # separate "Init" click first. features/layer_viewer/layer_viewer_
