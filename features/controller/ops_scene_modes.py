...
def _resolve_edit_target_mesh_readonly(context):
    """Read-only mirror of `_enter_edit_mode()`'s own target_mesh
    resolution, used only by `SUPERSKIN_OT_enter_layer_edit.poll()` so the
    button can stay enabled while an Armature (e.g. in Pose Mode) is the
    active object instead of only a mesh.

    Resolves via `object_selector.get_effective_mesh()` -- the SSOT for
    "which mesh is being managed right now" shared with the
    `deform_layer_viewer` domain (see docs/domains/controller.md) -- with
    `mw_absolute_last_mesh` (a genuinely separate, broader "nothing to go
    on at all" fallback, not part of the per-armature cache this SSOT
    migration removed) as the last resort. Read-only, same as before: no
    scene ID-property writes as a side effect of a poll() evaluated on
    every redraw."""
    ...
    candidate = context.view_layer.objects.get(abs_mesh_name)
...
def _enter_edit_mode(op, context):
    ...
    # Explicit override: takes priority over the SSOT resolution below
    # entirely. Set by callers that already know exactly which mesh the
    # user intends -- e.g. layer_viewer's Mesh dropdown
    # (`object_selector.get_effective_mesh()`) -- so a caller with more
    # specific knowledge than "whatever's active right now" can still win.
    # `op.mesh_name` only exists on SUPERSKIN_OT_enter_layer_edit; every
    # other caller has no such property and `getattr` falls through to "".
    ...
    # SSOT resolution: object_selector.get_effective_mesh() is the single
    # resolver for "which mesh is being managed right now", shared with the
    # deform_layer_viewer domain (see docs/domains/controller.md). No more
    # separate per-armature "last mesh" scene cache here -- the selector's
    # own remembered target_mesh (kept in sync from the viewport) covers
    # the same "Armature just became active" case this used to special-case.
    if not target_mesh:
    ...
    # Native Weight Guard: resolve any mismatch between the mesh's real
    # deform weights and SuperSkinPro's stored layer data now that `obj` is
    # confirmed to be the active object. Deliberately placed here rather
    # than in the calling Operator's execute() -- this is the earliest point
    # after target-mesh resolution (including the Pose-Mode/Armature-active
    # case above) where `obj` is guaranteed the active object, which
    # `CoreFacade(context)` calls below need. Skipped if `obj.mode` is
    # already 'EDIT' (a bare native-Tab entry -- no reliably flushed
    # `mesh.vertices` state to hash). `op.resolution` only exists on
    # SUPERSKIN_OT_enter_layer_edit (the dialog-driven entry point); every
    # other caller (OBJECT_OT_mw_enter_edit_mode, the toggle operator) has no
    # such property and silently defaults to 'KEEP_SSP' -- no dialog, just a
    # forced re-flatten -- since those are keymap/pie-menu entry points with
    # no invoke() wired up for a props dialog. See docs/domains/controller.md's
    # "Native Weight Guard" section.
    if obj.mode == 'OBJECT':
        try:
    ...
    # active_is_mask is intentionally left untouched here -- a fresh entry
    # now lands in whichever sub-mode (bone weight or Mask) was last active
    # on this mesh, not forced back to weight editing every time.
    #
    # This used to unconditionally reset to False on every entry ("Every
    # fresh entry into Edit Layer Weight defaults to bone editing, not Mask
    # editing"), specifically to avoid *silently* resuming Mask mode on the
    # next Enter Edit Mode after a Save. That rationale no longer applies:
    # the shared "[Object/Pose] [Edit Bone] [Edit Mask]" widget (see
    # interface/utils/mode_edit_toggle.py) now shows which sub-mode is
    # CURRENT/stored directly via which of its two buttons is depressed
    # before the click, so resuming is no longer silent -- it's exactly
    # what the button promised. Removed per explicit user request ("it
    # keeps resetting back to Edit Weight" -- the whole point of the
    # button remembering the sub-mode was defeated by this reset). See `is_mask` further down
    # (feeds `viz_mode`/`apply_active_bone()`) for why simply not resetting
    # here is also what makes the mesh actually start editing its mask, not
    # merely show the mask icon on the button while still editing bone
    # weight underneath.
    ...
    try:
        ...
        if not _has_layer_system(obj):
            # Auto-Init: seed the base Layer from this mesh's current real
            # Vertex Group weights right here, in the same Object-Mode
            # window _load_active_layer_to_temp_vgs() below already needs --
            # that call requires a layer system to already exist, so this
            # must run first and inside the same OBJECT-mode bounce (obj may
            # already be in EDIT mode at this point via a bare native Tab
            # entry -- see the poll() docstring above -- and CoreFacade's
            # mesh-ID writes are unsafe from inside Edit Mode).
            CoreFacade.debug_log("temp_vg", f"_enter_edit_mode() auto-init: seeding layer system on {obj.name!r}")
    ...
    # active_is_mask is whatever was last stored on this mesh (no longer
    # force-reset on entry -- see the comment above `obj = target_mesh`),
    # so _restore_addon_ui_for_edit_weight() below picks the 'MASK' or
    # 'SINGLE' visualizer to match, via its own fresh read of the flag.
...
def _restore_addon_ui_for_edit_weight(context, obj):
    """Restore the addon's own Edit Layer Weight UI to match a Blender Edit
    Mode session that already has (or should have) `__ssp_*` temp VGs
    present — the visualizer, the deform-bone overlay, the dimmed viewport
    theme, the N-panel tab, and `superskin_active_interface`.

    Shared tail of `_enter_edit_mode()` (a fresh entry) and the undo/redo
    handler that restores this same UI when Ctrl+Z/Ctrl+Shift+Z lands back
    inside a previously-exited Edit Layer Weight session — see
    `docs/bug-history/0032`. Blender's own undo/redo only restores mesh/
    object data (including the temp VGs themselves), never this addon's
    WindowManager-side interface state, so that has to be done explicitly.

    Does NOT call mode_set, load temp VGs, heal topology, or init the layer
    system — callers needing those (a real Enter Edit Mode) must do that
    first; the undo/redo path already has correct temp-VG/BMesh state
    restored natively by Blender before this runs.

    context.space_data can be None here (F3 search from a non-3D-viewport
    area, automation/headless invocation, or an undo/redo handler running
    outside the normal UI event loop) — see the matching guard and note in
    `_exit_edit_mode`. Skip the cosmetic overlay setup rather than crashing
    the whole restore after the real work below already succeeded.
    ...
    Save Weights). See also docs/domains/overlay_color.md's "Lifecycle is
    ...
    # (stacked on top of heal_topology_if_needed()/backfill_and_scan() in a
    # fresh Enter Edit Mode) was enough added latency that "Edit Layer
    # Weight" looked like it had stopped responding, unlike Tab (which
    # enters native Edit Mode with none of this addon-side setup at all —
    # see docs/bug-history for why that isn't actually the same code path
    # and can't regress this way). A row click already does this exact push
    # via BoneListAdapter.on_single_select() with the same deferral not
    # needed there since it isn't stacked on everything else this can be.
...
def _exit_edit_mode(op, context, *, keep_panel_open: bool = False):
    ...
    try:
        # Bake temp VGs → permanent storage before leaving Edit Mode
        _bake_temp_vgs_on_exit(context, obj)
        ...
        # Weight Brush hover cursor: hides the overlay circle and restores
        # the OS cursor (see weight_apply's public_api.py) immediately,
        # rather than waiting for the hover modal's own next MOUSEMOVE to
        # notice the mode changed — the mouse may not move at all between
        # this exit and whatever happens next, which would otherwise leave
        # a stale circle / hidden cursor on screen. See CLAUDE.md's
        # Viewport State Invariant.
        try:
            force_hide_brush_hover()
        except Exception:
            pass

        # Clear the shared HUD stack (Multi Color Mode / Mask mode / Bone
        # Picker labels) -- every current consumer only makes sense inside
        # Edit Layer Weight, so it must not linger into Object Mode. See the
        # Viewport State Invariant note in this domain's README.
        try:
        ...
        # Native Weight Guard: snapshot the mesh's real deform weights as the
        # new "last known SuperSkinPro-authored state" now that we're back in
        # Object Mode with mesh.vertices reliably flushed. See
        # docs/domains/controller.md's "Native Weight Guard" section.
        try:
            native_weight_guard.record_flatten_signature(obj)
        except Exception:
            pass
    ...
    # No more per-armature "last mesh" scene-cache write here --
    # object_selector's own target_mesh/target_armature (kept in sync from
    # the viewport) is the SSOT other resolution points read from instead.
    # See docs/domains/controller.md.
    ...
    # Force a distinct global undo checkpoint for the exit itself. Without
    # this, the bake -> mode_set(OBJECT) sequence above does not reliably
    # register as its own poppable undo step (observed via Blender's native
    # Edit > Undo History: no entry appears between the last in-Edit-Mode
    # weight op and whatever Object-Mode operator runs next) -- a single
    # Ctrl+Z after a subsequent Object-Mode operator (e.g. Mirror Weight All
    # Layer) then restores straight past this exit and back into Edit Mode,
    # skipping the "just exited, pre-next-op" state a user would expect to
    # land on first.
    try:
...
class OBJECT_OT_mw_force_pose_mode(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # ── Enter Pose Mode ──
        # Resolved via object_selector.get_effective_armature() -- the SSOT
        # shared with the deform_layer_viewer domain, replacing the old
        # per-armature "last mesh"/"last active armature" scene-cache pair
        # that used to be refreshed here (see docs/domains/controller.md).
        # Live active-object resolution takes priority in that function, so
        # selecting a different mesh (or Armature) and pressing Force Pose
        # Mode directly, without a prior formal Save/Exit, still resolves
        # correctly -- no manual cache refresh needed at this call site
        # anymore.
...
def _load_active_layer_to_temp_vgs(obj, context):
    ...
    try:
        ...
        # Split the Object-Mode pool into real-bone / orphan subsets before
        # seeding __ssp_pool -- only real bones get the undo-safe live temp
        # VG; orphans keep tracking via selected_orphan_names for the
        # duration of this Edit Mode session (see the pool bake-back in
        # _bake_temp_vgs_on_exit() for the reverse split).
        storage_obj = obj.superskin_storage
        full_pool = {n for n in storage_obj.selected_names.split(",") if n}
        real_names = {n for n in full_pool
                      if not n.startswith("__ssp_") and obj.vertex_groups.get(n)}
        storage_obj.selected_orphan_names = (
            f",{','.join(sorted(full_pool - real_names))}," if (full_pool - real_names) else ""
        )

        load_layer_to_temp_vgs(obj, layer_dict, mask_dict, active_idx, id_to_bone,
                                selected_pool_names=full_pool)
        _bump_temp_vg_session_token(obj.name)
    except Exception as e:
        print(f"[SuperSkinPro] Warning: could not load temp VGs: {e}")


def _bake_temp_vgs_on_exit(context, obj):
    """Flush EVERY resident Layer's own temp-VG set back to permanent
    storage when exiting Edit Mode -- not just the single active one, and
    not just weight/mask data but also any in-session structural CRUD
    (Add/Remove/Duplicate/Merge/Move/Rename/Visibility). Switching between
    Layers mid-session no longer bakes each one out immediately, and
    structural CRUD no longer writes ss_layers_meta immediately either (see
    core/ui_controller/layer_crud.py's _switch_temp_vg_layer() and the
    pending __ssp_meta_list mirror those CRUD ops now write to instead), so
    several Layers -- and a whole session's worth of structural changes --
    can be pending at once by the time a real exit happens; all of it must
    be flushed here or it would be silently discarded when delete_temp_vgs()
    wipes everything right after this call.

    Thin wrapper -- the actual implementation is
    CoreFacade.flush_edit_session() (core/ui_controller/layer_crud.py's
    flush_edit_session()), kept there rather than duplicated here so the
    Save Weights / Force Pose Mode / auto-save-guard paths that all funnel
    through this one function share a single implementation with any future
    core/-level caller. Caller must delete temp VGs in OBJECT mode after
    mode_set (unchanged contract)."""
    try:
        CoreFacade(context).flush_edit_session()
    except Exception as e:
...
class SUPERSKIN_OT_enter_layer_edit(bpy.types.Operator):
    ...
    mesh_name: bpy.props.StringProperty(
        ...
        description="Explicit mesh to enter Edit Layer Weight on, overriding the "
                     "active-object/per-armature-cache resolution below. Set by "
                     "callers that already know exactly which mesh the user "
                     "intends (e.g. layer_viewer's Mesh dropdown via "
                     "object_selector.get_effective_mesh()). Empty string (the "
                     "default) preserves the old resolution chain untouched, "
                     "for any caller that doesn't set it.",
        default="",
    )

    def invoke(self, context, event):
        # Native Weight Guard confirmation dialog disabled per explicit user
        # request: entry no longer blocks on a popup when native Weight
        # Paint edits conflict with SuperSkinPro's stored layer weights.
        # Always resolves KEEP_SSP, matching the no-dialog fallback every
        # other _enter_edit_mode() caller already uses (see
        # docs/domains/controller.md's "Native Weight Guard" section).
    ...
    def poll(cls, context):
        ...
        # (see ops_scene_modes.py's flip points and docs/core-interfaces/interface.md),
        ...
        #
        # No longer requires the layer system to already be initialised --
        # _enter_edit_mode() below auto-initialises it on first entry (see
        # the Object-Mode bounce inside that function), so this button stays
        # clickable on a never-initialised mesh instead of needing a
        # separate "Init" click first. features/deform_layer_viewer/layer_viewer/layer_viewer_
        # feature.py's button swaps its label to "Init & Edit" in that case
        # so the click's extra effect is still communicated.
        #
    ...
    def execute(self, context):
        # Resolution is applied inside `_enter_edit_mode()` itself (reading
        # `self.resolution` via `getattr(op, 'resolution', ...)`), not here --
        # that's the earliest point where the target mesh is guaranteed to
        # already be the active object, which the guard's CoreFacade calls
        # require. See the comment at that call site and
        # docs/domains/controller.md's "Native Weight Guard" section.
        return _enter_edit_mode(self, context)
...
def _schedule_auto_save(obj_name: str, expected_session_token) -> None:
    ...
    def _do_auto_save():
        ...
        try:
            ...
            if _obj.mode != 'OBJECT':
                bpy.ops.object.mode_set(mode='OBJECT')
            # Re-enter Edit Mode so flush_edit_session() can read temp VG weights.
            bpy.ops.object.mode_set(mode='EDIT')
            _bake_temp_vgs_on_exit(_ctx, _obj)
            try:
...
# ==============================================================================
# UNDO/REDO UI RESTORE — if Ctrl+Z / Ctrl+Shift+Z lands back inside a Blender
# Edit Mode session that already has __ssp_* temp VGs present (the addon-
# specific marker distinguishing a SuperSkinPro Edit Layer Weight session
# from plain native mesh-topology Edit Mode -- see docs/bug-history/0032),
# re-open this addon's own UI to match. Blender's undo/redo only restores
# mesh/object data, never this addon's WindowManager-side interface state
# (superskin_active_interface, the visualizer, the deform-bone overlay).
# ==============================================================================
...
def _superskin_undo_redo_restore_ui(*_):
    ...
    try:
        if not has_temp_vgs(obj):
            # Plain native Edit Mode (or an addon session with no in-progress
            # temp VGs, e.g. right after a clean Save) -- not our UI to own.
            return
    except Exception:
        return
    if ctx.window_manager.superskin_active_interface == 'SKINNING':
        return  # Already in sync -- nothing to restore.
    try:
