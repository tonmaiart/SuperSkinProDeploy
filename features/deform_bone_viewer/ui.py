...
from ...interface.utils.icons import get_clipboard_icon_id, get_clipboard_fill_icon_id
...
# ==============================================================================
# Clipboard drop-down menus (Plane Copy, ported from features/clipboard --
# see clipboard_logic.py and docs/domains/deform_bone_viewer.md)
# ==============================================================================

class SUPERSKIN_MT_deform_bone_weight_clipboard(bpy.types.Menu):
    """"Clipboard Bone Weight" drop-down -- Copy plus one menu entry per
    Add/Subtract/Replace paste mode (a menu item has nowhere to host a
    persistent mode dropdown, so the mode is baked into which item is
    clicked instead)."""
    bl_label = "Clipboard Bone Weight"
    bl_idname = "SUPERSKIN_MT_deform_bone_weight_clipboard"
...
class SUPERSKIN_MT_deform_layer_weight_clipboard(bpy.types.Menu):
    """"Clipboard Layer Weight" drop-down -- same shape as
    SUPERSKIN_MT_deform_bone_weight_clipboard, targeting the active
    layer's mask instead."""
    bl_label = "Clipboard Layer Weight"
    bl_idname = "SUPERSKIN_MT_deform_layer_weight_clipboard"
...
def draw_influence_list_system(layout, context, rows=8):
    ...
    draw_list_with_sidebar(
        ...
        button_defs=[
            ...
            ("separator", 1.0),
            ("menu", "SUPERSKIN_MT_deform_bone_weight_clipboard",
             lambda ctx: get_clipboard_icon_id() or 'BONE_DATA', ""),
            ("menu", "SUPERSKIN_MT_deform_layer_weight_clipboard",
             lambda ctx: get_clipboard_fill_icon_id() or 'RENDERLAYERS', ""),
            ("separator", 2.2),
        ],
    )
...
def register():
    ...
    bpy.utils.register_class(SUPERSKIN_MT_deform_bone_weight_clipboard)
    bpy.utils.register_class(SUPERSKIN_MT_deform_layer_weight_clipboard)


def unregister():
    bpy.utils.unregister_class(SUPERSKIN_MT_deform_layer_weight_clipboard)
    bpy.utils.unregister_class(SUPERSKIN_MT_deform_bone_weight_clipboard)
