...
class SUPERSKIN_OT_toggle_mask_mode(bpy.types.Operator):
    """Toggle Mask editing on/off without disturbing the Deform Bones
    list's own bone selection (last_clicked_index / active_orphan_name /
    selected_names are left untouched either way). Replaces the former
    virtual Mask row -- mirrors BoneListAdapter.on_single_select()'s two
    branches exactly, just without going through write_selection()."""
