...
class OBJECT_OT_mw_copy_skin_weight_maya(bpy.types.Operator):
    ...
    def _world_positions(self, context, obj):
        """World-space vertex positions, aligned to obj.data.vertices index
        order — read straight from `obj.data.vertices`, the mesh's
        stored/bind (rest pose) shape, unaffected by whatever pose the bound
        Armature (or any other modifier) currently has it deformed into."""
