...
class BonePickerFeature(UnifiedFeatureExtension):
    ...
    def execute(self, action: str, context, core_facade: CoreFacade) -> dict:
        try:
            if action == "start_bone_picker":
                ...
                if storage:
                    # Mode-aware -- routes to __ssp_pool in Edit Mode
                    # instead of writing storage.selected_names directly
                    # (not reliably undo-tracked while in Edit Mode).
    ...
    def get_keymap_items(self) -> list:
        """Expose Alt+2's ``(km, kmi, label)`` to the in-panel shortcut
