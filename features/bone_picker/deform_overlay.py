...
# Yellow HUD label shown while the modal is held -- requested on the shared
# HUD stack (CoreFacade.request_hud_slot/release_hud_slot, see
# docs/core-interfaces/facade-api.md) from set_holding() below, instead of this module
# owning its own bottom-of-screen draw handler for it, so it can never
# overlap another domain's line (e.g. overlay_color's pink "Multi Color").
_HUD_LABEL = "BONE PICKER"
_HUD_COLOR = (1.0, 0.8, 0.0, 1.0)  # yellow
_HUD_SLOT = 2  # third row from the anchor -- see docs/core-interfaces/facade-api.md's reserved-slot table
...
def _draw_callback():
    ...
    # Multi-select pool -- read via the same __ssp_pool BMesh mechanism the
    # Deform Bones list uses, so undo/redo of a sweep-select reverts the
    # green wedge color correctly. storage.selected_names itself is not
    # reliably undo-tracked while in Edit Mode (custom ID Property, same
    # class of gap docs/bug-history/0016 fixed for weight data).
    try:
        import bmesh as _bm_mod
        from ...core.layer_storage.temp_vg_bridge import read_pool_names_from_bm
        bm = _bm_mod.from_edit_mesh(obj.data)
        pool_names = read_pool_names_from_bm(bm, obj)
    except Exception:
        pool_names = set()
    orphan_pool = getattr(storage, "selected_orphan_names", "") if storage else ""
    pool_names |= {n for n in orphan_pool.split(",") if n}
