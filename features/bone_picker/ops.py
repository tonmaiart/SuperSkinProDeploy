...
def _clear_mask_state(obj):
    """Mask is just another row now, not a separate mode."""
...
class OBJECT_OT_mw_pick_bone(bpy.types.Operator):
    ...
    vertex_search_radius: bpy.props.FloatProperty(
        ...
        description="Maximum radius to search for a vertex under the cursor after the raycast hits the mesh surface",
    ...
    def cancel(self, context):
        """Blender calls this (not modal()) when it force-terminates a running modal operator
        outside its own RIGHTMOUSE/Release-2/ESC exit paths above."""
        global _bone_picker_active
        _bone_picker_active = False
        self._restore_cursor(context)
        _deform_overlay.clear_hover()
    ...
    def _set_picker_cursor(self, context):
        """Explicit save-then-override: record that *this* session pushed a modal cursor
        override, then push it."""
    ...
    def _sweep_add_bone(self, context, bone_name):
        ...
        if not storage:
            return
        # session_pool mirrors the live pool in memory, so membership checks
        # skip re-parsing storage.selected_names.
