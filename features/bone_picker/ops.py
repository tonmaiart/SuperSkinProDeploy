...
# Minimum interval (seconds) between re-running the hover-detection scan
# (_get_hovered_bone()/_iter_nearby_bones()) in modal()'s MOUSEMOVE handler.
# That scan projects every armature bone's head/tail to 2D screen space (and
# falls back to a full scene.ray_cast() + per-polygon-vertex projection loop
# when no bone is close enough) -- cheap for one call, but native MOUSEMOVE
# events can arrive far faster than the viewport actually redraws (high
# poll-rate mice/trackpads easily exceed 60Hz), and re-running the scan on
# every single event was the main source of the reported picker lag. The
# crosshair cursor itself (set_hover()'s mouse_pos arg) is still updated on
# every event regardless, so the cursor stays perfectly smooth even while
# the heavier bone scan is throttled to this cadence.
...
class OBJECT_OT_mw_pick_bone(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'MOUSEMOVE':
            ...
            # Throttle the expensive per-bone scan (see _HOVER_SCAN_INTERVAL
            # above); the crosshair position below is still updated every
            # event so cursor movement itself stays perfectly smooth.
            ...
            if self.is_sweep_add:
                ...
                if not self._multi_mode and self.hovered_bone and self.hovered_bone != self._last_hovered:
                    ...
                    if vg:
                        ...
                        ctrl = CoreFacade(context)
                        # This is a passive preview, not a real commit -- the
                        # real commit (_select_single_bone() on TWO+RELEASE
                        # below) pushes its own checkpoint unsuppressed, so
                        # Ctrl+Z after a pick is unaffected. See
                        # CoreFacade.suppress_meta_undo_push()'s docstring
                        # and docs/bug-history/0007.
    ...
    def invoke(self, context, event):
        ...
        # Mask editing has no bones to pick -- a mask isn't tied to any one
        # bone, so bone-picking makes no sense while the "Edit Mask"
        # sub-mode is active. This operator's own bone-pick modal never
        # starts in that case (the former Alt+2 redirect into the now-
        # removed layer_picker domain was removed along with it).
