...
# Same URL as interface/panel_main.py's _DOCS_URL. Deliberately duplicated,
# not imported -- interface/* internals are a closed subsystem features are
# forbidden from importing directly (see interface/README.md), the same
# reasoning that used to justify duplicating _addon_version() here.
    ...
    def draw_activate_prompt(self, layout, context) -> None:
        ...
        inside the box:
          ...
          3. an icon-only `QUESTION` button first, then a "Where can I get
             ...
             at the docs site for now, until there's a dedicated
