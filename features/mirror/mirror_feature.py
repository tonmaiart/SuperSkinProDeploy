...
# ==============================================================================
# MirrorAllLayersFeature — UnifiedFeatureExtension
# ==============================================================================

class MirrorAllLayersFeature(UnifiedFeatureExtension):
    """Unified extension for the "Mirror Weight All" tool-socket entry.

    Reuses SSPrefMirror / MirrorPreferencesService / SUPERSKIN_PT_mirror_options
    wholesale (same options popover as the Mirror section) — this domain owns
    no settings of its own, only a different action: mirror every layer in
    the stack instead of just the active one. Deliberately kept in this same
    package/module rather than a separate features/ package, since it has
    nothing to add beyond sharing the Mirror domain's own state.

    **Deliberately plugged into LAYER_SOCKET, not SKINNING_SOCKET** — the
    SKINNING tab body only ever draws while
    ``window_manager.superskin_active_interface == 'SKINNING'``
    (`interface/panel_main.py::_draw_skin_tab()`), which in practice means
    only while genuinely inside a live "Edit Layer Weight" session (that
    flag is flipped by `_enter_edit_mode()`/`_exit_edit_mode()`, not by
    Blender's native Object/Edit mode toggle — see `docs/core-interfaces/interface.md`'s
    "Interface State" section). Putting a bulk, every-layer operator behind
    that tab would make it reachable almost exclusively from Edit Mode. The
    LAYER tab, by contrast, is what's shown by default and stays visible in
    every mode (`docs/core-interfaces/interface.md` again), so LAYER_SOCKET is the tab
    that's actually reachable from a plain Object Mode session without first
    entering Edit Layer Weight — matching `object.mirror_weights_all_layers`'s
    own `context.mode == 'OBJECT'`-only `poll()` (see `ops.py`). This split
    matters because looping `switch_to_layer()` across every layer while
    genuinely in Blender Edit Mode bakes each visited layer's `__ssp_*` temp
    VG back to `ss_layer_N` (an ID Property `docs/bug-history/0016` confirms
    Blender's BMesh undo does NOT track) — so most of the layers a mid-Edit-Mode
    run would touch could not be reverted by a single Ctrl+Z the way the
    single-layer "Mirror Weights" button's edit can. Restricting this
    operator to Object Mode (where every layer write already goes straight
    to `ss_layer_N` with no temp-VG/BMesh-undo entanglement at all) avoids
    that undo-granularity mismatch entirely instead of trying to reproduce
    Blender-native-undo-safe behavior across a multi-layer bake loop.
    """

    # ── Configuration (class attributes) ───────────────────────────────────

    domain_id = "mirror_all_layers"
    actions = ["mirror_all"]
    section_title = "Mirror Weight All"
    draw_tab = "LAYER_SOCKET"  # plugged into tool_socket's LAYER dropdown, see docs/domains/tool_socket.md -- Object-Mode-only, see class docstring
    link = "https://docs.superskinpro.com/mirror_tab/"
    ...
    def draw_section(self, layout, context) -> None:
        """Draw the Mirror Weight All section: same button + gear-popover row
        shape as the Mirror section (reusing SUPERSKIN_PT_mirror_options), just
        wired to the all-layers operator instead of the active-layer one."""
        row_btn = layout.row(align=True)
        row_btn.scale_y = 1.2
        row_btn.operator("object.mirror_weights_all_layers", text="Mirror Weight All Layer")
        row_btn.popover(SUPERSKIN_PT_mirror_options.bl_idname, text="", icon='PREFERENCES')
...
def register():
    ...
    UnifiedRegistry.register(MirrorAllLayersFeature())


def unregister():
    """Unregister PropertyGroups and the extension."""
    UnifiedRegistry.unregister("mirror_all_layers")
