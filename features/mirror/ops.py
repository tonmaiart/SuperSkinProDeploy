...
PropertyGroups and MirrorPreferencesService live in mirror_feature.py; UI
drawing (including the SUPERSKIN_UL_mirror_sr UIList this file used to
register -- a UIList is display-only, not an action) moved to
features/skin_tools_ui/ui_mirror.py, see docs/domains/skin_tools_ui.md.
"""
...
class OBJECT_OT_MirrorWeights(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        all_layers = MirrorPreferencesService.get_mirror_all_layers()
        try:
            ...
            if do_layer:
                ...
                # before, since nothing at all would happen. The bone-pair
                # plan is layer-independent (vertex-group names/bone centers
                # only), so this single check up front is representative of
                # every layer an all_layers run is about to touch.
                ...
                if not pairs:
                    ...
                    self.report(
                        ...
                        + ("was mirrored, on every layer. " if all_layers else "was mirrored. ")
                        + "Check that a matching Armature modifier is present "
                        "and bone names match a search/replace pair.",
                    )
        except ValueError:
            return {'CANCELLED'}
