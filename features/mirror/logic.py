...
def apply_mask_flat(mask_dict: dict, vertex_coords: list,
    ...
    self-contained raw coordinate test per ``docs/domains/mirror.md``'s
...
def build_source_side_surface_raw(core_facade, axis_idx, direction):
    ...
    Per ``docs/domains/mirror.md``'s guardrail: the weight and mask
...
def _fill_mask_gaps(mask_dict_before, result, gap_v_indices, vertex_coords, axis_idx, surface, mask_default):
    ...
    *surface* must be from ``build_source_side_surface_raw()`` — the mask
    ...
    see ``docs/domains/mirror.md``'s guardrail).
    """
    if not gap_v_indices or surface is None:
        return
...
def _prepare_mirror_pipeline(core_facade, axis, axis_idx, direction, sr_raw, mirror_data):
    ...
    # surface further down — see ``docs/domains/mirror.md``'s guardrail:
...
def execute_mirror_pipeline_all_layers(core_facade):
    ...
    Triggered via ``SSPrefMirror.mirror_all_layers`` (the "Mirror to All
    Layers" checkbox in the Mirror Options popover) instead of a separate
    button/domain — see ``docs/domains/mirror.md``.
    ...
    **Safe to call during a live Edit Layer Weight session, not just Object
    Mode:** every read/write call this function (and ``_mirror_active_layer()``)
    makes is mode-aware — ``core_facade.switch_to_layer()`` uses the
    per-Layer resident temp-VG session cache (no bake-to-permanent-storage on
    a plain switch, see ``docs/core-interfaces/facade-api.md``), weight
    reads/writes go through ``read_active_layer()``/``write_active_layer()``,
    mask through ``get_active_mask_dict()``/``write_mask_dict()``, and the
    layer-index/mask_default lookups here and in ``_mirror_active_layer()``
    use ``get_meta_list()``, which is mode-aware, so a layer
    added/removed/reordered earlier in the same Edit Mode session is seen
    correctly instead of a stale pre-session snapshot.
    ...
    # get_meta_list() is mode-aware -- this loop must see any layer
    # add/remove/duplicate/merge/move made earlier in the same Edit Mode
    # session, not a stale pre-session snapshot. See
    # docs/core-interfaces/facade-api.md's get_meta_list() entry.
