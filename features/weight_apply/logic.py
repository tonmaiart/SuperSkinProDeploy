...
def compute_chain_bone_ids(core_facade, active_vg_id, id_to_bone, deform_bone_ids):
    ...
    candidate set", so a rig with no usable chain reconstruction never
    blocks Scale outright.
    """
...
def compute_nearest_bones(core_facade, selected_verts, candidate_bone_ids, id_to_bone,
    ...
    that scope before calling this. Target selection is deliberately a
    strict two-step pipeline -- hierarchy chain first (by the caller, before
    this function ever runs), then raw 3D nearest-distance (here) -- with no
    step anywhere that requires a candidate bone to already carry weight.
    Whether a bone already has weight, at this vertex or nearby, plays no
    part in whether it is eligible to receive Scale's freed weight; it only
    affects the redistribution split `rust_scale_logic` performs once the
    eligible set is decided (proportional if some already carry weight,
    even if none do). An earlier revision additionally narrowed the raw
    nearest-set with a topology-corroboration pass (requiring a candidate to
    already have real weight somewhere in a small mesh-adjacency vicinity of
    the selection) to catch a spatially-close-but-not-mesh-connected bone
    (two fingers touching, an ear near the head); that pass was removed
    (2026-09-14, per explicit user request removing every "must already have
    weight" gate from Scale's targeting) once hierarchy chain scoping
    (ancestor/descendant only, added after that pass) started catching the
    same class of case for any rig whose reconstructed hierarchy separates
    the two touching parts into different branches.
...
def apply_scale(layer_dict, mask_dict, selected_verts, active_vg_id, intensity,
    ...
    *nearest_bone_ids* (`{v_idx: [bone_id, ...]}`, from `compute_nearest_bones()`
    ...
    docs/domains/weight_apply.md`'s "Scale's Redistribution Target Scope" for the full algorithm.
...
#  `docs/domains/weight_apply.md`'s Sharpen sections for the full design
...
def _expand_hops(core_facade, target_verts, passes):
    ...
    docs/domains/weight_apply.md's `dirty_verts` contract and the "untouched
...
def apply_sharpen(layer_dict, mask_dict, selected_verts, coords, adjacency,
    ...
    Rust module's doc comments, and `docs/domains/weight_apply.md`'s
