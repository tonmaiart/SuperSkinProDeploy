...
def _on_smooth_affected_only_changed(self, context):
    from ...core.facade import CoreFacade
    CoreFacade.save_prefs()
    # Color/slot/owner-id for this HUD line live in draw.py, the module that
    # actually owns this domain's GPU/HUD drawing (see its persistent
    # sync handler for why a manual toggle alone isn't enough to keep this
    # correct across an Edit Layer Weight exit/re-entry).
    from . import draw
...
class WeightApplyFeature(UnifiedFeatureExtension):
    ...
    section_title = "Apply"
    # No "Apply:" caption drawn above the section body anymore (2026-09-13,
    # per explicit user request) -- see docs/domains/weight_apply.md's "No
    # section label" note. `link` below therefore has no header row left to
    # attach its INFO button to (see UnifiedFeatureExtension.show_section_label's
    # own docstring) and is effectively unreachable from this panel now.
        ...
        if action == "add":
            ...
            if not is_mask:
                ...
                if nb_cache is not None and "nearest_bone_ids" in nb_cache:
                    ...
                    # Step 1 -- hierarchy scoping: restrict candidates to
                    ...
                    # never-block-Scale-outright philosophy compute_nearest_
                    # bones() itself follows for a totally empty result.
                    ...
                    # Step 2 -- nearest-distance: of the hierarchy-scoped
                    # candidates from step 1, pick whichever sit spatially
                    # closest to each vertex. Neither step checks whether a
                    # candidate already carries weight anywhere -- eligibility
                    # is purely hierarchy + distance (2026-09-14, per explicit
                    # user request removing every "must already have weight"
                    # gate from this targeting pipeline).
    ...
    def _finish_write(self, action: str, core_facade: CoreFacade, ctx: dict,
        ...
        with core_facade.profile_section(f"weight_apply.{action}.merge", size=len(dirty_verts)):
            ...
            # Shallow top-level copy only: every vertex NOT in res_layer_diff
            # keeps sharing its existing inner dict object with ctx["layer_int"]
            # instead of being deep-copied. Safe because nothing downstream of
            # this dict ever mutates a per-vertex inner dict in place --
            # write_active_layer_from_calc()'s layer_int_for_convert slice
            # (core/facade/write.py) only reads it before crossing the Rust FFI
            # boundary (which itself copies, per this project's FFI Marshaling
            # Findings), _write_active_layer_string()'s map_layer_to_string()
            # builds a brand-new string-keyed dict rather than editing this one,
            # and the Object-Mode branch below only reads via .items(). Every
            # vertex actually touched this call gets a fresh dict from
            # res_layer_diff via .update() below, so this is still a full,
            # complete, per-vertex-correct snapshot -- only the copy strategy
            # changed, not what data ends up in it. See superskinpro-core-optimize
            # skill's Step 2 lever table for the same pattern applied elsewhere.
        ...
        with core_facade.profile_section(f"weight_apply.{action}.write", size=len(dirty_verts)):
            if is_mask:
                ...
                # to storage and does not flatten/redraw, so Object Mode still
                # needs write_active_layer()'s finish()-calling path -- but
                # calling that method directly would mean building res_layer_str
                # here (int->string, full mesh) just to have write_active_layer()
                # immediately convert it BACK to int (string->int, full mesh)
                # before handing it to _write_active_layer_string(), which then
                # converts to string a THIRD time internally
                # (map_layer_to_string()) -- three full-mesh conversions of the
                # same data, the first two of which are a pure round-trip that
                # exactly cancels out. full_layer_int is already int-keyed with
                # this same id_to_bone, so call _write_active_layer_string() +
                # finish() directly (mirroring write_active_layer()'s own body)
                # instead of going through write_active_layer()'s string-typed
                # entry point -- same as the is_mask branch above already does.
                ctrl = core_facade.get_ctrl()
        ...
        # reads `result.get("status")`, so this is backward compatible. Kept
        # so a caller dispatching several apply_action() calls in a row
        # against the SAME ctx can fold each call's actual written result
        # back into its own local ctx and let the next call read the
        # just-applied state -- real accumulation across calls -- without
        # re-running the full `snapshot_context()` (a BMesh read) every time.
        return {
            "status": "FINISHED",
    ...
    def get_keymap_items(self) -> list:
        """Expose the gesture/repeat bindings' ``(km, kmi, label)`` triples
        to the in-panel shortcut editor
        (``interface/utils/keymap_editor.py``) -- see
        ``UnifiedFeatureExtension.get_keymap_items()`` for the contract.
        Includes the Weight Brush tool's own toggle shortcut
        (``brush/brush_tool.py``) alongside the gesture/repeat bindings --
        the hover-preview binding (``brush/keymap.py``) is deliberately not
        surfaced here, same as before this method existed."""
        from . import keymap as _keymap
        from .brush import brush_tool as _brush_tool
        return _keymap.get_registered_keymap_items() + _brush_tool.get_registered_keymap_items()

    # ── UI layout ─────────────────────────────────────────────────────────

    def draw_section(self, layout, context) -> None:
        """UI moved to features/skin_tools_ui/ui_weight_apply.py — see
    ...
    def populate(self, data: dict) -> None:
        ...
        if BRUSH_ENABLED:
            from .brush.brush_ops import populate_prefs
    ...
    def serialize_into(self, full_dict: dict) -> None:
        ...
        from .brush import BRUSH_ENABLED
        if BRUSH_ENABLED:
            from .brush.brush_ops import serialize_prefs
...
def unregister():
    ...
    # HUD-slot release lives in draw.cleanup() (called right after this from
    # __init__.py's unregister()), alongside the persistent sync handler it
    # also owns -- see draw.py's "Smooth Affected Only" HUD sync section.
