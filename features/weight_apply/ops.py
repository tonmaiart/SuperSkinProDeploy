...
# Ease-in precision taper near the neutral (0.0) start of a drag, per
# explicit user request -- the first stretch of the axis (low Add/Smooth
# intensity, or Scale intensity near "no change") is where small
# differences matter most and are hardest to land precisely at full,
# constant pixel-to-value sensitivity. Below `_GESTURE_EASE_IN_RANGE`
# (measured as |self._drag_value|, so it applies symmetrically on both
# sides of 0 -- Add ramping up from 0 and Scale ramping down from 0 alike),
# sensitivity is scaled down toward `_GESTURE_EASE_IN_MIN_FACTOR` right at
# 0.0 and linearly ramps back up to full (1.0x) speed at the range boundary,
# so the same mouse movement lands on a finer intensity step near the start
# of the drag than it does further out. This is a passive, always-on taper
# on top of the existing sensitivity -- distinct from (and stacks with)
# Ctrl's discrete `_FINE_MODE_DIVISOR` fine-sub-grid mode below, which is
# still the only way to get fine precision away from 0.
...
class SUPERSKIN_OT_weight_gesture(bpy.types.Operator):
    ...
    future deltas use (`_FINE_MODE_DIVISOR`) -- it never touches
    `self._drag_value` or warps the cursor differently, so the transition is
    snap-free. A persistent background thread (`_precompute_worker()`,
    ...
    def _apply_result(self, context, real_action, action_data, intensity, result):
        ...
        if write_result.get("status") != "CANCELLED":
            # Ctrl (fine mode) appends "(Slow)" to whatever text would
            # already be shown.
