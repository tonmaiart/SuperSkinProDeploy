...
During either gesture, holding Ctrl switches to a finer precision grid --
see ops.py:SUPERSKIN_OT_weight_gesture for the full contract.
...
Alt+Shift+LMB entry point, now back to plain Alt+RMB. Alt+Shift+RMB used to
be claimed by `circle_tool_adjust`'s own radius/tool-toggle gesture (which
is why plain Alt+RMB, not Alt+Shift+RMB, was picked here); that domain has
since been removed entirely (per explicit user request, once
`lasso_tool_adjust` replaced Box/Circle in `weight_apply`'s own tool-select
row — see `docs/domains/lasso_tool_adjust.md`), so Alt+Shift+RMB is
currently unclaimed by anything in this addon.
...
def get_registered_keymap_items():
    ...
    by ``idname`` plus creation-rank, since several items here share the
    same idname (``superskin.weight_gesture``); see that module's
