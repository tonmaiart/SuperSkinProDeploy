...
def get_effective_armature(context):
    """Return the Armature layer_viewer/controller should treat as the
    active rig: the active object itself if it's already an Armature, the
    active mesh's Armature modifier target if the active object is a mesh,
    otherwise the last Armature remembered by the Armature/Mesh selectors
    (``target_armature``, kept in sync from the viewport -- see the module
    docstring). Returns None if none of those resolve.

    Live active-object resolution intentionally takes priority over the
    remembered fallback, mirroring ``get_effective_mesh()``'s own
    precedence -- this is what keeps the answer correct even when the user
    switches directly from one rig's mesh/Armature to a different rig's
    without the panel ever redrawing to resync ``target_armature``."""
    obj = context.active_object
    ...
    if obj and obj.type == 'MESH':
        ...
        if armature is not None:
            return armature
    return context.window_manager.superskin_layer_viewer_selector.target_armature
...
def draw_object_selectors(layout, context):
    ...
    further controls to the same row instead of starting a new one. Must
    stay usable even when the active object is an Armature (or nothing at
    all).
    ...
    # (2026-09-14, per explicit user request) -- both share
    # `deform_layer_viewer_feature.py`'s top row, so a mismatched scale_y
    # here reads as a shorter, misaligned dropdown next to that button.
    row.scale_y = 1.4
    # No fixed ui_units_x anymore -- this row is now the sole item drawn
    # into `mesh_col` (layer_viewer_feature.py's `draw_section()`), a
    # dedicated column with no sibling widget beside it, so Blender
    # stretches it to fill that column's own width automatically -- which
    # is what makes it match the Layer List's own column width (see that
    # call site's docstring). An earlier revision pinned this to a fixed
    # `ui_units_x = 7` back when the Mesh dropdown shared a row with the
    # "[Object/Pose] [Edit Bone] [Edit Mask]" widget buttons, to stop a long/short mesh vs.
    # Layer name from shifting those buttons -- moot now that this control
    # has its own row with nothing beside it to shift.
