...
class SUPERSKIN_OT_layer_enter_pose_mode(bpy.types.Operator):
    ...
    The "this button can't be reached mid-Edit-Layer-Weight-session" premise
    this operator used to rely on (LAYER tab hides once
    ``superskin_active_interface`` flips to 'SKINNING') stopped holding once
    ``deform_layer_viewer_feature.py``'s ``_draw_combined()`` merge made this
    exact widget row draw on the SKINNING tab too, always -- which IS visible
    during a real session with pending ``__ssp_*`` temp-VG edits. When
    `has_pending_edit_mode_changes()` is True, this delegates to
    ``object.mw_force_pose_mode`` by bl_idname (not a Python import -- same
    "Cross-domain button/operator references" precedent already used for
    "Edit Layer Weight") instead of refusing, since that operator already
    bakes temp VGs back to the active layer and restores the Edit Layer
    Weight viewport-state snapshot before switching -- exactly what "Save
    Weights" does. Both operators resolve the target Armature through the
    same ``object_selector.get_effective_armature()`` SSOT (see
    docs/domains/controller.md), so the delegated call agrees with this
    one's own resolution without any scene-cache hand-off. This bl_idname
    reference will need to move (like the "Edit Layer Weight" one) whenever
    `controller` is actually deleted.
    """
    ...
    def execute(self, context):
        ...
        if obj and obj.type == 'MESH' and obj.mode == 'EDIT':
            if CoreFacade(context).has_pending_edit_mode_changes():
                # Bake in-progress temp-VG edits + restore viewport state via
                # the controller domain's own operator (see class docstring)
                # instead of refusing the mode switch outright.
...
class SUPERSKIN_OT_layer_toggle_visible_by_item(bpy.types.Operator):
    ...
    When this row is part of an active multi-selection (2+ layers picked
    via Ctrl/Shift-click on the Layers list, the same pool "Merge Selected
    Layers" reads), the click applies to every selected layer at once --
    all selected layers end up hidden if the clicked layer was visible,
    or shown if it was hidden. Clicking a row outside the current
    selection (or with 0/1 layers selected) only toggles that one row, as
    before.
    """
            ...
            for idx in targets:
                if new_visible is not None:
                    ...
                    if item is not None and item.visible == new_visible:
                        continue
                _run_preserving_edit_session(context, ctrl.toggle_visible, idx)
            sync_layers_to_ui_collection(obj)
...
class SUPERSKIN_OT_layer_add(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def _do_add():
            ...
            new_idx = _run_preserving_edit_session(context, ctrl.create_layer, name)
...
class SUPERSKIN_OT_layer_remove(bpy.types.Operator):
    """Remove the active layer, or every currently multi-selected layer at
    once when 2+ layers are selected via the Layers list multi-select pool
    (``layer_selected_indices`` -- the same pool "Merge Selected Layers" and
    "Duplicate" read). An explicit ``layer_index`` override (e.g. a future
    per-row call site) always targets just that one layer regardless of the
    current selection. The sidebar's Remove button never passes an
    override, so it follows the current selection.
    """
    ...
    def execute(self, context):
        ...
        if self.layer_index == -1 and len(selected_indices) >= 2:
            def _do_multi():
                ctrl = CoreFacade(context)
                # Tracked by name, not index -- each removal reindexes the
                # remaining stack (same reasoning as merge_selected's
                # disabled_names handling), so a not-yet-processed selected
                # index can no longer be trusted once an earlier removal has
                # run.
                names = [
                ...
                _run_preserving_edit_session(context, _remove_all)
        ...
        def _do():
            ...
            _run_preserving_edit_session(context, _remove)
            _select_only_layer(obj, ctrl.active_layer_index)
            sync_layers_to_ui_collection(obj)
            _enforce_visualizer_from_tab_state(context)
...
class SUPERSKIN_OT_layer_move(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def _do():
            ...
            moved = _run_preserving_edit_session(context, ctrl.move_layer, resolved_target, self.direction)
...
class SUPERSKIN_OT_layer_duplicate(bpy.types.Operator):
    """Duplicate the active layer, or every currently multi-selected layer
    at once when 2+ layers are selected via the Layers list multi-select
    pool (``layer_selected_indices`` -- the same pool "Merge Selected
    Layers" reads). An explicit ``layer_index`` override always targets
    just that one layer regardless of the current selection.
    """
    ...
    def execute(self, context):
        ...
        if self.layer_index == -1 and len(selected_indices) >= 2:
            def _do_multi():
                ...
                new_indices = []
                offset = 0
                # Ascending order + running offset: duplicating a layer
                # inserts the copy right after it, shifting every later
                # index up by 1 -- so each not-yet-processed selected
                # index needs +offset to still point at the right layer.
                for idx in sorted(selected_indices):
                    actual = idx + offset
                    new_idx = _run_preserving_edit_session(context, ctrl.duplicate_layer, actual)
        ...
        def _do():
            ...
            new_idx = _run_preserving_edit_session(context, ctrl.duplicate_layer, resolved_target)
            if new_idx is not None and new_idx >= 0:
                _select_only_layer(obj, new_idx)
            sync_layers_to_ui_collection(obj)
            _enforce_visualizer_from_tab_state(context)
...
class SUPERSKIN_OT_layer_merge_selected(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def _do():
            ...
            ok = _run_preserving_edit_session(context, _do_merge)
            if not ok:
                return False
...
class SUPERSKIN_OT_layer_icon_apply(bpy.types.Operator):
    """Set the layer's icon COLOR -- the active layer, or every currently
    multi-selected layer at once when 2+ layers are selected via the
    Layers list multi-select pool (``layer_selected_indices``, the same
    pool "Merge Selected Layers"/"Duplicate" read). Invoked once per
    color-swatch entry at the end of the Layer list's "More" menu (see
    ``SUPERSKIN_MT_layer_list_more_options.draw()``, ``ui.py``) -- clicking
    an entry both applies the color and closes that menu. The icon actually
    drawn also depends on the layer's live mask data (White/Black/Edited
    state, see ``CoreFacade.get_layer_mask_state()``) -- this operator only
    ever changes the color half of it."""
    bl_idname = "superskin.layer_icon_apply"
    bl_label = "Set Layer Icon Color"
    bl_options = {'INTERNAL', 'UNDO'}
    # One of interface/utils/icons.py's LAYER_ICON_COLORS (e.g. 'blue'),
    # never a built-in Blender icon identifier -- see ui.py's
    # _LAYER_ICON_MENU_CHOICES.
    ...
    def execute(self, context):
        ...
        def _do():
            ...
            for idx in targets:
                _run_preserving_edit_session(context, ctrl.set_layer_icon, idx, self.icon_name)
            sync_layers_to_ui_collection(obj)
            return len(targets)
