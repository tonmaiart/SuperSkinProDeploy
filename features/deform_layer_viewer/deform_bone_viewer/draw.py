...
"[Object/Pose] [Edit Bone] [Edit Mask]" widget's "Edit Mask" button,
``superskin.toggle_mask_mode`` in ``ops.py`` (formerly a standalone "Mask"
toggle button, see ``docs/domains/deform_layer_viewer.md``) -- but also
flippable directly from ``ui.py``'s mask-row selection logic, so this
module cannot simply hook one call site and must keep polling).

Always requests a line on the shared HUD stack (see
``docs/core-interfaces/facade-api.md``'s "Shared HUD Stack" section) while a mesh is in
...
_HUD_SLOT = 0  # bottom-most row -- see docs/core-interfaces/facade-api.md's reserved-slot table
