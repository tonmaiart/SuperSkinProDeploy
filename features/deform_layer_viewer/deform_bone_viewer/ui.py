...
def _extra_keep_predicate_impl(context, data, item, original_idx):
    """Draw-time row filter for the unified (real + orphan + mask) mirror
    list. Orphan (and mask) rows are always kept regardless of
    ``bone_list_filter_mode`` -- the former standalone 'Orphan' filter
    state was removed (folded into 'Influence', which already showed
    orphans unconditionally, per explicit user request)."""
...
class BoneListAdapter(ListSelectionAdapter):
    ...
    def read_selection(self, context, obj):
        ...
        # Mode-aware: in Edit Mode reads the undo-safe __ssp_pool BMesh
        # state (plus selected_orphan_names) instead of selected_names,
        # which is not reliably undo-tracked while in Edit Mode.
        from ....core.layer_storage.temp_vg_bridge import has_temp_vgs, read_pool_names_from_bm
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            import bmesh as _bm_mod
            bm = _bm_mod.from_edit_mesh(obj.data)
            selected_keys = read_pool_names_from_bm(bm, obj)
            selected_keys |= {n for n in storage.selected_orphan_names.split(",") if n}
        else:
    ...
    def write_selection(self, context, obj, selected_keys, last_key, history):
        ...
        # Mode-aware: in Edit Mode writes real bones to the undo-safe
        # __ssp_pool BMesh state, orphans to selected_orphan_names. Outside
        # Edit Mode, writes selected_names directly (memfile undo already
        # handles that correctly there).
        from ....core.layer_storage.temp_vg_bridge import has_temp_vgs, write_pool_names_bm
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            real_names = {n for n in selected_keys
                          if n in name_to_idx and not n.startswith("__ssp_")}
            orphan_names_sel = selected_keys - real_names
            write_pool_names_bm(obj, obj.data, real_names)
            storage.selected_orphan_names = (
                f",{','.join(sorted(orphan_names_sel))}," if orphan_names_sel else ""
            )
        elif selected_keys:
...
def _focus_view_on_bone(context, obj, bone_name):
    ...
    Panning only (sets ``RegionView3D.view_location``) -- deliberately does
    NOT touch object mode, selection, or zoom, so a double-click has no side
    effects beyond where the view is looking. Resolves the armature via the
    mesh's own Armature modifier (same convention as `_armature_for_mesh()`
    in layer_viewer/object_selector.py) rather than requiring the user to
    already have the armature active.
    """
    arm_obj = None
...
class SUPERSKIN_OT_select_vertex_group_row(bpy.types.Operator):
    ...
    with the Layers tab operator. Double-click recenters the 3D viewport on
    the clicked bone (see `_focus_view_on_bone()` above) -- same manual
    double-click detection technique as the Layers list's double-click-to-
    rename, shared via `interface/template_ui/select_ops.py`'s
    `is_double_click()`/`reset_double_click()`.
    """
...
class MESH_UL_influence_list_view(SuperSkinListMixin, bpy.types.UIList):
    ...
    def is_selected(self, context, data, key: str) -> bool:
        """Mode-aware: in Edit Mode, reads the undo-safe __ssp_pool BMesh
        state (plus selected_orphan_names) instead of selected_names, which
        is not reliably undo-tracked while in Edit Mode (custom ID Property,
        same class of gap docs/bug-history/0016 fixed for weight data)."""
        obj = data
        from ....core.layer_storage.temp_vg_bridge import has_temp_vgs, read_pool_names_from_bm
        if obj.mode == 'EDIT' and has_temp_vgs(obj):
            import bmesh as _bm_mod
            bm = _bm_mod.from_edit_mesh(obj.data)
            if key in read_pool_names_from_bm(bm, obj):
                return True
            orphan_pool = obj.superskin_storage.selected_orphan_names
            return f",{key}," in orphan_pool if orphan_pool else False
        return f",{key}," in obj.superskin_storage.selected_names
...
class SUPERSKIN_MT_bone_list_more_options(bpy.types.Menu):
    """"More" overflow menu for the Deform Bones list sidebar (per explicit
    user request) -- holds the Clipboard Bone Weight actions (Copy/Cut/
    Paste Add/Subtract/Replace), drawn flat instead of behind a nested
    "Clipboard Bone Weight" submenu (per a later explicit user request --
    the former SUPERSKIN_MT_deform_bone_weight_clipboard submenu class was
    removed, its five menu entries inlined directly here). No section
    label header either (per a still-later explicit user request removing
    the "Clipboard Bone Weight" label line) -- the five entries just start
    the menu directly."""
    bl_label = "More"
...
def _bone_list_filter_icon(context):
    ...
    return 'BONE_DATA' if mode == 'INFLUENCE' else 'LONGDISPLAY'
...
def draw_influence_list_system(layout, context, rows=8, obj=None):
    """Draw the Deform Bones list with a search box and a button toolbar
    above it.

    *obj* lets the caller pass the mesh to display explicitly (e.g.
    ``layer_viewer/object_selector.py``'s ``get_effective_mesh()``, which
    may differ from ``context.active_object`` while an Armature is active
    in Pose Mode) -- mirrors ``layer_viewer/ui.py``'s ``draw_layer_list()``
    convention. Falls back to ``context.active_object`` when omitted.

    No longer accepts a ``top_fn`` hook (removed 2026-09-13, per explicit
    user request) -- it used to exist for embedding the Layer list here,
    which was already abandoned in favor of drawing both lists as sibling
    blocks (see ``docs/domains/deform_layer_viewer.md``), leaving the hook
    itself unused by any caller."""
    if obj is None:
        obj = context.active_object
    ...
    # No virtual Mask row to key off anymore -- staleness now means "the
    # mirror collection hasn't been built at all yet even though the mesh
    # has vertex groups to show" (e.g. right after an addon reload, before
    # any operator has run).
    ...
    draw_list_with_sidebar(
        ...
        rows=rows,
        button_defs=[
            # Single cycle button (Show All <-> Influence) replacing the
            # former three-way "enum_radio" set (2026-09-15, per explicit
            # user request -- the removed 'Orphan' state is folded into
            # 'Influence', which already always kept orphan rows visible,
            # see _extra_keep_predicate_impl() above). depress=True reads
            # as a toggle color while Influence is active.
            ("operator", "superskin.cycle_bone_list_filter",
             _bone_list_filter_icon, "", {}, _bone_list_filter_is_influence),
            ("separator", 1.0),
            # The flexible flush-right ("spacer",) entry that used to sit
            # here (pushing "More" to the toolbar's far right edge) was
            # removed 2026-09-13, per explicit user request, once this
            ...
            # width-constrained split() column was overflowing the button
            # row past that column's actual boundary (reported as the
            # toolbar's buttons visibly spilling into the neighboring list/
            # panel edge). A plain fixed separator keeps "More" directly
            # after the filter buttons instead of flush-right.
            #
            # Clipboard Bone Weight moved into this "More" overflow menu
            # (per explicit user request) instead of being its own direct
            # sidebar button.
