...
vertices correctly; see docs/domains/deform_layer_viewer.md's "Why Plane
...
class _PlaneClipboard:
    ...
    def cut(self, ctrl: CoreFacade) -> None:
        """Copy, then clear the source data. Since ``copy()`` is always
        dense across the whole mesh regardless of selection (see the module
        docstring's "Why Plane Copy must be dense"), the clear is dense too
        -- it is not scoped to whatever happens to be selected, matching
        this clipboard's own ignore-the-selection copy semantics."""
        self.copy(ctrl)
