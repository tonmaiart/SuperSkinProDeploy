...
DeformBoneViewerFeature's copy_bone_plane/cut_bone_plane/paste_bone_plane_*/
copy_layer_plane/cut_layer_plane/paste_layer_plane_* actions (see
clipboard_logic.py). Ported from the former Plane Copy tab in
features/clipboard -- see docs/domains/deform_layer_viewer.md.

Paste is one operator per mode (Add/Subtract/Replace) rather than one
operator plus a shared mode dropdown -- each is drawn as its own flat
entry directly inside each list's "More" overflow menu (ui.py's
SUPERSKIN_MT_bone_list_more_options / layer_viewer/ui.py's
SUPERSKIN_MT_layer_list_more_options; no nested "Clipboard Bone Weight" /
"Clipboard Layer Weight" submenu anymore, per explicit user request) -- a
menu item has nowhere to host a persistent dropdown control, so the mode
is baked into which item you click instead.
"""
...
class SUPERSKIN_OT_deform_cut_bone_weight(bpy.types.Operator):
    ...
    bl_description = "Copy the active Vertex Group's weight densely across the whole mesh, then clear it from every vertex"
...
class SUPERSKIN_OT_deform_cut_layer_weight(bpy.types.Operator):
    ...
    bl_description = "Copy the active layer's mask densely across the whole mesh, then clear it entirely"
    bl_options = {'REGISTER', 'UNDO'}
