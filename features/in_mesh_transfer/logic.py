...
package, and `docs/core-interfaces/core-subsystems.md`'s Import Invariant #3 forbids
...
def _build_restricted_surface(positions, triangles, allowed_verts):
    ...
    whole mesh) — see `docs/domains/weight_transfer.md` rule 18 for why a
...
def mark_source(core_facade):
    ...
    If a Source is already marked on this mesh, this call CLEARS it (returns
    `None`). Otherwise it snapshots the current selection as the new Source
