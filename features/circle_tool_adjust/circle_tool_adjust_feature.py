...
class CircleToolAdjustFeature(UnifiedFeatureExtension):
    ...
    keymaps = [
        {
            "key": "Alt+Shift+RMB", "label": "Circle Radius / Toggle Select Tool",
            "mode": "Hold", "source_label": "Circle Select Radius / Toggle",
        },
    ]
    ...
    def get_keymap_items(self) -> list:
        """Expose Alt+Shift+RMB's ``(km, kmi, label)`` to the in-panel
        shortcut editor (``interface/utils/keymap_editor.py``) -- see
