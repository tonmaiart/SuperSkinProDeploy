...
# ==============================================================================
# Mesh dropdown menu -- plain text button instead of the default ID
# PointerProperty widget (which always draws an object-type icon and a
# clear "X" button once a value is set). Selecting an entry just reselects
# the object in the viewport; the existing viewport->dropdown sync
# (_needs_sync / _sync_from_viewport, above) picks up the new active object
# into target_mesh/target_armature on the next redraw, so no separate
# write-back is needed here.
# ==============================================================================

class SUPERSKIN_MT_layer_viewer_mesh_select(bpy.types.Menu):
    bl_label = "Select Mesh"
    bl_idname = "SUPERSKIN_MT_layer_viewer_mesh_select"
    ...
    def draw(self, context):
        ...
        if not meshes:
            layout.label(text="No mesh available")
            return
        for obj in sorted(meshes, key=lambda o: o.name):
            op = layout.operator("superskin.layer_viewer_select_mesh", text=obj.name)
            op.mesh_name = obj.name


class SUPERSKIN_OT_layer_viewer_select_mesh(bpy.types.Operator):
    """Select ``mesh_name`` in the viewport, driven by the plain Mesh
    dropdown menu above."""
...
def draw_object_selectors(layout, context):
    ...
    row.menu("SUPERSKIN_MT_layer_viewer_mesh_select", text=mesh_label)
    return row
...
def register():
    ...
    bpy.utils.register_class(SUPERSKIN_MT_layer_viewer_mesh_select)
...
def unregister():
    ...
    bpy.utils.unregister_class(SUPERSKIN_MT_layer_viewer_mesh_select)
