...
class SUPERSKIN_MT_layer_rename_overflow(bpy.types.Menu):
    """Overflow menu for secondary layer-list actions (merge, clean-up).
    ...
    "Clean-up This Mesh" / "Clean-up All Meshes" moved in here from the
    section's old bottom Init/Clean-up row (see layer_viewer_feature.py) --
    both pop a confirm dialog via their own operator's invoke()."""
    bl_label = "More"
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.operator("superskin.layer_remove_data",
                        text="Clean-up This Mesh", icon='TRASH')
        layout.operator("superskin.layer_remove_all_data",
    ...
    *obj* lets the caller pass the mesh to display explicitly (e.g.
    ...
    draw_list_with_sidebar(
        ...
        button_defs=[
            ...
            ("operator", "superskin.layer_duplicate", 'DUPLICATE', "", {}),
            ("separator", 1.0),
