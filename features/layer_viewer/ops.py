...
class SUPERSKIN_OT_layer_enter_pose_mode(bpy.types.Operator):
    ...
    place (see docs/core-interfaces/interface.md's Interface State section). This is
            ...
            for idx in targets:
                if new_visible is not None:
                    ...
                    if item is not None and item.visible == new_visible:
                        continue
                _run_in_object_context(context, ctrl.toggle_visible, idx)
            sync_layers_to_ui_collection(obj)
    ...
    def execute(self, context):
        ...
        if self.layer_index == -1 and len(selected_indices) >= 2:
            def _do_multi():
                ...
                _run_in_object_context(context, _remove_all)
        ...
        if self.layer_index == -1 and len(selected_indices) >= 2:
            def _do_multi():
                ...
                for idx in sorted(selected_indices):
                    actual = idx + offset
                    new_idx = _run_in_object_context(context, ctrl.duplicate_layer, actual)
