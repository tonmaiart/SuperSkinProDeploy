...
"""Vendored update-checking engine (CGCookie blender-addon-updater)."""
...
def _remove_or_move_aside(path):
    """Delete ``path``; a loaded native module cannot be deleted on Windows but can be renamed."""
    try:
        os.remove(path)
    except OSError:
...
class SingletonUpdater:
    ...
    def deep_merge_directory(self, base, merger, clean=False):
        ...
        # Stale native binaries must not survive an update; the release ships
        # the full bin/ tree.
        base_bin = os.path.join(base, "bin")
        if os.path.isdir(os.path.join(merger, "bin")) and os.path.isdir(base_bin):
            for path, _dirs, files in os.walk(base_bin):
                for file in files:
                    try:
                        _remove_or_move_aside(os.path.join(path, file))
                    except OSError:
                        self.print_verbose("Failed to clear " + file)
                        self.print_trace()
                ...
                if os.path.isfile(dest_file):
                    ...
                    for pattern in self._overwrite_patterns:
                        if fnmatch.filter([file], pattern):
                            ...
                            break
                    if replaced:
                        _remove_or_move_aside(dest_file)
    ...
    def reload_addon(self):
        ...
        if not self._auto_reload_post_update:
            CoreFacade.debug_log("feature_domains", "addon_updater: restart Blender to complete update")
            return
        ...
        if "addon_disable" in dir(bpy.ops.wm):  # 2.7
            ...
            CoreFacade.debug_log("feature_domains", "addon_updater: 2.7 reload complete")
        else:  # 2.8
            ...
            CoreFacade.debug_log("feature_domains", "addon_updater: 2.8 reload complete")
    ...
    def check_for_update_async(self, callback=None):
        ...
        if not self._check_interval_enabled:
            ...
            CoreFacade.debug_log(
                "feature_domains",
                "addon_updater: running background check for update")
...
_REPO_CONFIG_DEFAULTS = {
    ...
    "subfolder_path": "",
    "verbose": False,
    "overwrite_patterns": ["*.py", "*.so", "*.pyd", "*.dll", "*.png", "*.jpg",
