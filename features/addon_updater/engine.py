...
"""Vendored update-checking engine (CGCookie blender-addon-updater), lift-and-shift
from the former root-level ``addon_updater.py`` into the ``addon_updater`` feature
domain.

See original documentation for usage: https://github.com/CGCookie/blender-addon-updater
"""
...
from ... import ADDON_PACKAGE, ADDON_VERSION
from ...core.facade import CoreFacade

# This module now lives two directories below the addon root
# (features/addon_updater/engine.py), whereas it used to live directly at the
# addon root. The vendored path logic below (reinstall, updater status JSON
# location) assumes a root-level location, so this constant preserves that
# assumption after the move.
_ADDON_ROOT = str(Path(__file__).resolve().parent.parent.parent)

# Network timeout (seconds) for update-check/download requests. Neither
# urlopen() call site below originally passed a `timeout=`, so a slow or
# silently-blocked connection (e.g. a firewall dropping packets instead of
# rejecting the connection outright) could hang the background check thread
# for minutes instead of failing fast -- worth bounding explicitly now that
# `auto_check_update` defaults to True and this fires on every session/
# reload rather than only when a user opts in.
_REQUEST_TIMEOUT_SECONDS = 10


def _rmtree_safe(path):
    """Recursively delete ``path``, clearing the read-only attribute first.

    On Windows, git marks packed ``.git/objects`` files read-only, so a plain
    ``shutil.rmtree`` raises ``PermissionError`` whenever ``path`` (the
    staging folder or the addon root itself) contains a ``.git`` directory
    checked out on this platform -- e.g. when the addon is installed via a
    local git clone rather than a packaged zip.
    """
...
class SingletonUpdater:
    ...
    def __init__(self):
        ...
        # Get data from the running blender module (addon). Use the root
        # addon package (ADDON_PACKAGE), not this submodule's own __package__,
        # since reinstall logic below and the updater's own status JSON
        # filename must all resolve to the actual installed addon location,
        # not features/addon_updater/.
        self._addon = ADDON_PACKAGE.lower()
        self._addon_package = ADDON_PACKAGE  # Must not change.
        # Deliberately OUTSIDE _ADDON_ROOT -- a Blender Extension's install
        # directory can be wiped/replaced on update (deep_merge_directory()
        # below literally does this), and multiple installs of this same
        # addon under different extension IDs (e.g. dev-testing the updater
        # itself against several disposable IDs) used to each leave their
        # own "<id>_updater/" folder behind inside the addon's own working
        # tree. get_dev_output_dir() resolves to a per-user Blender data
        # directory instead (same rationale as profiler/debug_console/
        # support_report, see docs/core-interfaces/facade-api.md's "Dev
        # Output Folders" section) -- one shared folder, with each distinct
        # _addon_package still getting its own uniquely-named JSON file
        # inside it (see get_json_path() below), so multiple installs still
        # can't collide with each other's state.
        self._updater_path = CoreFacade.get_dev_output_dir("addon_updater")
        self._addon_root = _ADDON_ROOT
        self._json = dict()
        self._error = None
    ...
    def get_raw(self, url):
        ...
        try:
            if context:
                result = urllib.request.urlopen(request, context=context, timeout=_REQUEST_TIMEOUT_SECONDS)
            else:
                result = urllib.request.urlopen(request, timeout=_REQUEST_TIMEOUT_SECONDS)
        except urllib.error.HTTPError as e:
            CoreFacade.debug_log("feature_domains", f"addon_updater.get_raw(): HTTPError {e.code} for url={url!r}")
            if str(e.code) == "403":
            ...
            CoreFacade.debug_log("feature_domains", f"addon_updater.get_raw(): URLError {e.reason} for url={url!r}")
            reason = str(e.reason)
    ...
    def get_api(self, url):
        ...
        if get is not None:
            try:
                ...
                self._update_ready = None
                CoreFacade.debug_log("feature_domains", f"addon_updater.get_api(): invalid JSON response: {e}")
                print(self._error, self._error_msg)
                self.print_trace()
                return None
        else:
    ...
    def stage_repository(self, url):
        ...
        error = None
        CoreFacade.debug_log("feature_domains", f"addon_updater.stage_repository(): entry url={url!r}")
        ...
        if error is not None:
            ...
            CoreFacade.debug_log("feature_domains", f"addon_updater.stage_repository(): aborted, {error}")
            return False
        ...
        try:
            ...
            self.url_retrieve(urllib.request.urlopen(request, context=context, timeout=_REQUEST_TIMEOUT_SECONDS),
                              self._source_zip)
            # Add additional checks on file size being non-zero.
            self.print_verbose("Successfully downloaded update zip")
            CoreFacade.debug_log("feature_domains", "addon_updater.stage_repository(): download successful")
            return True
        except Exception as e:
            self._error = "Error retrieving download, bad link?"
            self._error_msg = "Error: {}".format(e)
            CoreFacade.debug_log("feature_domains", f"addon_updater.stage_repository(): download exception: {e}")
        ...
        with zipfile.ZipFile(self._source_zip, "r") as zfile:
            ...
            for name in zfile.namelist():
                ...
                if name.endswith(zsep):
                    ...
                    # Some zip sources (e.g. a release zip whose archive step
                    # never wrote an explicit directory entry for a folder
                    # that has exactly one file in it) list a nested file
                    # without ever listing its parent as its own entry. Do
                    # not assume the mkdir branch above already ran for this
                    # file's parent -- create it here too if needed.
                    dest_path = os.path.join(outdir, sub_path)
                    os.makedirs(os.path.dirname(dest_path), exist_ok=True)
                    with open(dest_path, "wb") as outfile:
                        data = zfile.read(name)
                        outfile.write(data)
                        self.print_verbose("Extract - create: " + dest_path)
    ...
    def deep_merge_directory(self, base, merger, clean=False):
        ...
        # Path to be aware of and not overwrite/remove/etc. `self._updater_path`
        # now lives outside `base` entirely (see Updater.__init__), so every
        # `== self._updater_path` / `in [self._updater_path]` check below is a
        # permanently-false no-op -- harmless, kept only because this is
        # vendored code and there's no longer anything inside `base` for it to
        # match against.
            ...
            try:
                ...
                for f in folders:
                    if os.path.join(base, f) == self._updater_path:
                        continue
                    _rmtree_safe(os.path.join(base, f))
                    self.print_verbose(
        ...
        # but avoid removing/altering the updater file.
    ...
    def check_for_update_async(self, callback=None):
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.check_for_update_async(): entry")
        is_ready = (
        ...
        if is_ready:
            ...
            CoreFacade.debug_log(
                "feature_domains",
                f"addon_updater.check_for_update_async(): using cached result, version={self._update_version!r}")
            # Cached update.
            callback(True)
            return
    ...
    def check_for_update_now(self, callback=None):
        CoreFacade.debug_log("feature_domains", "addon_updater.check_for_update_now(): entry")
        self._error = None
        self._error_msg = None
        self.print_verbose(
    ...
    def check_for_update(self, now=False):
        ...
        CoreFacade.debug_log("feature_domains", f"addon_updater.check_for_update(): entry now={now}")
        ...
        if self._update_ready is not None and not now:
            CoreFacade.debug_log(
                "feature_domains",
                f"addon_updater.check_for_update(): returning cached result update_ready={self._update_ready}")
            return (self._update_ready,
        ...
        if not now and not self.past_interval_timestamp():
            ...
            CoreFacade.debug_log(
                "feature_domains",
                "addon_updater.check_for_update(): skipped, check interval not reached")
        ...
        if self._fake_install:
            ...
            CoreFacade.debug_log(
                "feature_domains",
                "addon_updater.check_for_update(): fake_install=True, returning fake update-ready result")
            return (self._update_ready,
                    self._update_version,
                    self._update_link)
        ...
        if len(self._tags) == 0:
            ...
            CoreFacade.debug_log(
                "feature_domains",
                "addon_updater.check_for_update(): no tags/releases found, up to date")
            return (False, None, None)
        ...
        if new_version == ():
            ...
            CoreFacade.debug_log(
                "feature_domains",
                "addon_updater.check_for_update(): could not parse a version from latest tag, treating as up to date")
            return (False, None, None)
            ...
            if not self._include_branch_auto_check:
                ...
                CoreFacade.debug_log(
                    "feature_domains",
                    f"addon_updater.check_for_update(): branch target available, version={new_version!r}")
            ...
            if new_version > self._current_version:
                ...
                CoreFacade.debug_log(
                    "feature_domains",
                    f"addon_updater.check_for_update(): update found, version={new_version!r}")
                return (True, new_version, link)
        ...
        self._update_version = None
        self._update_link = None
        CoreFacade.debug_log("feature_domains", "addon_updater.check_for_update(): up to date")
        return (False, None, None)
    ...
    def run_update(self, force=False, revert_tag=None, clean=False, callback=None):
        ...
        CoreFacade.debug_log(
            "feature_domains",
            f"addon_updater.run_update(): entry force={force} revert_tag={revert_tag!r} clean={clean}")
        ...
        if self._fake_install:
            ...
            CoreFacade.debug_log("feature_domains", "addon_updater.run_update(): fake_install path completed")

        elif not force:
            if not self._update_ready:
                self.print_verbose("Update stopped, new version not ready")
                CoreFacade.debug_log("feature_domains", "addon_updater.run_update(): aborted, update not ready")
                if callback:
                    callback(
                        self._addon_package,
                ...
                CoreFacade.debug_log("feature_domains", "addon_updater.run_update(): aborted, no update link")
                if callback:
            ...
            if not res:
                ...
                CoreFacade.debug_log(
                    "feature_domains", f"addon_updater.run_update(): staging failed: {self._error_msg}")
            ...
            if res < 0:
                CoreFacade.debug_log(
                    "feature_domains", f"addon_updater.run_update(): unpack/install failed: {self._error_msg}")
                if callback:
                    callback(self._addon_package, self._error_msg)
                return res
            ...
            if not res:
                print("Error in staging repository: " + str(res))
                CoreFacade.debug_log(
                    "feature_domains", f"addon_updater.run_update(): staging failed (forced): {self._error_msg}")
                if callback:
                    callback(self._addon_package, self._error_msg)
                return self._error_msg
            res = self.unpack_staged_zip(clean)
            if res < 0:
                CoreFacade.debug_log(
                    "feature_domains", f"addon_updater.run_update(): unpack/install failed (forced): {self._error_msg}")
                return res
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.run_update(): completed successfully")
    ...
    def set_updater_json(self):
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.set_updater_json(): entry")
    ...
    def save_updater_json(self):
        ...
        CoreFacade.debug_log("feature_domains", "addon_updater.save_updater_json(): entry")
        ...
        try:
            ...
            CoreFacade.debug_log("feature_domains", f"addon_updater.save_updater_json(): FAILED to write {jpath}")
    ...
    def async_check_update(self, now, callback=None):
        ...
        try:
            ...
            CoreFacade.debug_log("feature_domains", f"addon_updater.async_check_update(): exception: {exception}")
...
def read_current_version():
    """Return this addon's ADDON_VERSION (root __init__.py) as an int tuple.

    Replaces the former root __init__.py "_fake_bl_info" hack: the standard
    features/__init__.py registration loop calls register() with no
    arguments, so this domain must source its own version instead of
    receiving a bl_info dict.
    """
    return tuple(int(p) for p in ADDON_VERSION.split("."))


_REPO_CONFIG_PATH = os.path.join(os.path.dirname(__file__), "repo_config.json")

# Fallback values used only if repo_config.json is missing or malformed, so a
# bad hand-edit of that file degrades gracefully instead of breaking updater
# registration entirely (matches the resilience philosophy of ops.py's
# SingletonUpdaterNone fallback for a broken engine import).
_REPO_CONFIG_DEFAULTS = {
    "engine": "Github",
    "user": "tonmaiart",
    "repo": "SuperSkinPro-Extension",
    "website": "https://github.com/tonmaiart/SuperSkinPro-Extension",
    "private_token": None,
    "subfolder_path": "",
    "verbose": False,
    "overwrite_patterns": ["*.py", "*.so", "*.png", "*.jpg", "*.json",
                            "README.md", "LICENSE", "blender_manifest.toml"],
    "remove_pre_update_patterns": ["*.py", "*.pyc", "__pycache__"],
    "include_branches": True,
    "use_releases": True,
    "include_branch_list": ["master"],
    "manual_only": False,
    "fake_install": False,
    "show_popups": True,
    "version_min_update": [0, 0, 0],
...
def read_repo_config():
    """Load features/addon_updater/repo_config.json.

    This is the developer-facing repository configuration (GitHub user/repo/
    token, overwrite patterns, branch list, etc.) -- edit that JSON file
    directly instead of this module. Falls back to hardcoded safe
    defaults on any read/parse error so a bad edit doesn't stop the updater
    from registering at all.
    """
    try:
        with open(_REPO_CONFIG_PATH, "r", encoding="utf-8") as fh:
            config = json.load(fh)
        CoreFacade.debug_log("feature_domains", "addon_updater.read_repo_config(): loaded repo_config.json")
        return config
    except Exception as e:
        CoreFacade.debug_log(
            "feature_domains",
            f"addon_updater.read_repo_config(): FAILED to load repo_config.json, using built-in defaults: {e}")
        return dict(_REPO_CONFIG_DEFAULTS)


def configure(skip_tag=None, select_link=None):
    """Configure the Updater singleton from repo_config.json.

    Called once from addon_updater_feature.py's register(). skip_tag/
    select_link are passed in rather than imported directly from ops.py, so
    this module has no hard dependency on the ops/UI layer above it (matches
    the Deep Matrix Reload bottom-up order: engine -> ops -> feature).
    """
    if Updater.error:
        print("Exiting updater configuration, " + Updater.error)
        return
    CoreFacade.debug_log("feature_domains", "addon_updater.configure(): entry")
    Updater.clear_state()  # Clear internal vars, avoids reloading oddities.

    cfg = read_repo_config()

    # Confirm your updater "engine" (Github is default if not specified).
    Updater.engine = cfg.get("engine", "Github")

    # If using private repository, indicate the token here.
    Updater.private_token = cfg.get("private_token")

    # Choose your own username, must match website (not needed for GitLab).
    Updater.user = cfg.get("user", "")

    # Choose your own repository, must match git name for GitHub and Bitbucket,
    # for GitLab use project ID (numbers only).
    Updater.repo = cfg.get("repo", "")

    # updater.addon = # defined at top of ops.py module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    Updater.website = cfg.get("website", "")

    # Addon subfolder path. Default is "" or None, meaning root.
    Updater.subfolder_path = cfg.get("subfolder_path", "")

    # Used to check/compare versions.
    Updater.current_version = read_current_version()

    # Optional, consider turning off for production or allow as an option
    # This will print out additional debugging info to the console
    Updater.verbose = cfg.get("verbose", False)

    # Patterns for files to actively overwrite if found in new update file and
    # are also found in the currently installed addon.
    Updater.overwrite_patterns = cfg.get(
        "overwrite_patterns",
        ["*.py", "*.so", "*.png", "*.jpg", "*.json", "README.md", "LICENSE", "blender_manifest.toml"])

    # Patterns for files to actively remove prior to running update.
    Updater.remove_pre_update_patterns = cfg.get(
        "remove_pre_update_patterns", ["*.py", "*.pyc", "__pycache__"])

    # Allow branches like 'master' as an option to update to, regardless
    # of release or version.
    Updater.include_branches = cfg.get("include_branches", True)
    ...
    Updater.use_releases = cfg.get("use_releases", True)

    # Populate if using "include_branches" option above.
    Updater.include_branch_list = cfg.get("include_branch_list") or None  # None == ['master']

    # Only allow manual install, thus prompting the user to open
    # the addon's web page to download, specifically: updater.website
    Updater.manual_only = cfg.get("manual_only", False)

    # Used for development only, "pretend" to install an update to test
    # reloading conditions.
    Updater.fake_install = cfg.get("fake_install", False)
    ...
    Updater.show_popups = cfg.get("show_popups", True)

    # Set the min and max versions allowed to install.
    version_min = cfg.get("version_min_update", [0, 0, 0])
    Updater.version_min_update = tuple(version_min) if version_min is not None else None
    version_max = cfg.get("version_max_update")
    Updater.version_max_update = tuple(version_max) if version_max is not None else None

    # Override with a custom function on what tags to skip showing for
    # updater; passed in from ops.py to avoid a reverse import dependency.
    if skip_tag is not None:
        Updater.skip_tag = skip_tag
    if select_link is not None:
        Updater.select_link = select_link
    ...
    # blender crashes).
    Updater.auto_reload_post_update = cfg.get("auto_reload_post_update", False)
