...
docs/domains/debug_console.md. This domain's 8 real operators are
...
(now the docked SUPERSKIN_PT_preferences panel) but was removed outright
once the compact control became the only update-checking entry point
users need. draw_tab is
...
class SUPERSKIN_PT_addon_update_confirm(bpy.types.Panel):
    ...
    settings gear icon used to use for the old `SUPERSKIN_PT_settings_popup`
    (now a docked `SUPERSKIN_PT_preferences` panel instead), per explicit
    ...
    codebase (`features/mirror`, `features/debug_console`,
    `features/weight_transfer`): a Panel registered against the `UI` region
