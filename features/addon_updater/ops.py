...
# Two-stage confirm chain, three operators/panels, each stage handing off
# to the next via a DIFFERENT bl_idname (never re-invoking itself):
#
#   SUPERSKIN_PT_addon_update_confirm (Panel popover, addon_updater_feature.py)
#     "Later"             -> AddonUpdaterInstallPopup(confirmed=False)  [EXEC_DEFAULT, no-op]
#     "Update and Restart" -> AddonUpdaterQuitConfirm                  [INVOKE_DEFAULT, opens stage 2]
#   AddonUpdaterQuitConfirm (this operator's own invoke_popup(), below)
#     "Cancel"                              -> AddonUpdaterInstallPopup(confirmed=False)  [EXEC_DEFAULT, no-op]
#     "Update and Close Blender Immediately" -> AddonUpdaterInstallPopup(confirmed=True)   [EXEC_DEFAULT, does the real work]
#   AddonUpdaterInstallPopup (below) -- the actual confirmed-gated action, no UI of its own
#
# AddonUpdaterInstallPopup deliberately has NO invoke()/draw(): a floating
# `invoke_popup()` on THIS class was tried first and reverted, since a
# button calling its own bl_idname defaults to INVOKE_DEFAULT and would
# re-open the same popup on every click instead of running execute() -- see
# weight_apply/ui.py's `_draw_op_row()` for the same gotcha hit before. Each
# stage below only ever targets a DIFFERENT operator's bl_idname, so
# INVOKE_DEFAULT is exactly what's wanted at each handoff (it's what lets
# AddonUpdaterQuitConfirm's own invoke() fire when stage 1 calls it).
class AddonUpdaterInstallPopup(bpy.types.Operator):
    """Confirm updating and restarting Blender now, or cancel"""
    bl_label = "Update {x} addon".format(x=updater.addon)
    bl_idname = updater.addon + ".updater_install_popup"
    bl_description = "Download and install the update, restarting Blender when done"
    bl_options = {'REGISTER', 'INTERNAL'}

    # Set by whichever calling button was actually clicked (stage 1's
    # "Later", or stage 2's "Cancel" / "Update and Close Blender
    # Immediately") -- all of them call this SAME operator with a different
    # preset, so execute() cannot assume "invoked" means "confirmed".
    confirmed: bpy.props.BoolProperty(default=False, options={'HIDDEN'})

    def execute(self, context):
        CoreFacade.debug_log(
            "feature_domains",
            f"addon_updater.AddonUpdaterInstallPopup.execute(): entry confirmed={self.confirmed}")
...
# Second-stage confirm -- "are you REALLY sure" popup between stage 1's
# "Update and Restart" and the actual install, warning explicitly that
# Blender closes immediately with no save. Its own invoke_popup() (a
# custom-styled floating popup, per explicit request -- NOT Blender's
# native invoke_props_dialog() modal, since that dialog's own built-in
# OK/Cancel footer can't be recolored, and a red confirm button was
# explicitly wanted here) is legitimate and safe: nothing calls THIS
# operator's own bl_idname from inside its draw(), only
# AddonUpdaterInstallPopup's, so there is no re-invoke-loop risk.
class AddonUpdaterQuitConfirm(bpy.types.Operator):
    """Second confirmation: installing now closes Blender immediately, unsaved"""
    bl_label = "Confirm Update"
    bl_idname = updater.addon + ".updater_quit_confirm"
    bl_description = "Confirm installing the update -- Blender will close immediately, unsaved"
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def draw(self, context):
        ...
        col.scale_y = 0.85
        col.label(text="The update will close Blender", icon="ERROR")
        col.label(text="immediately, without saving.")
        col.label(text="Are you sure?")
        col.separator(factor=1.2)

        row = layout.row(align=True)
        row.scale_y = 1.4
        # EXEC_DEFAULT here targets AddonUpdaterInstallPopup, which has no
        # invoke() of its own -- see the module comment above
        # AddonUpdaterInstallPopup for why this is required, not optional.
        row.operator_context = 'EXEC_DEFAULT'
        cancel = row.operator(AddonUpdaterInstallPopup.bl_idname, text="Cancel")
        cancel.confirmed = False
        go_row = row.row(align=True)
        go_row.alert = True
        go = go_row.operator(
            AddonUpdaterInstallPopup.bl_idname,
            text="Update and Close Blender Immediately",
        )
        go.confirmed = True

    def execute(self, context):
        # Reached only via redo-panel/repeat-last, never a normal click path
        # (draw()'s two buttons both target AddonUpdaterInstallPopup
        # instead, see above) -- a no-op is the only safe default so
        # "repeat last action" can't silently re-trigger a real install.
        return {'CANCELLED'}
...
# global var used to prevent duplicate popup handlers
...
def background_update_callback(update_ready):
    """Passed into the updater as the launch-time background check's
    callback (see check_for_update_background() below).

    No longer force-opens an install popup -- the "Update" button at the
    bottom of the LAYER tab (addon_updater_feature.py's
    draw_update_button()) is the only notification surface now, and it
    reads updater.update_ready directly on its next draw. This callback's
    only remaining job is nudging a redraw so that button appears right
    away instead of waiting for an incidental repaint. Still gated by
    `show_popups` (repo_config.json) since a background-detected update is
    exactly the kind of unsolicited UI surfacing that flag exists to
    suppress.
    """
    CoreFacade.debug_log(
...
def post_update_callback(module_name, res=None):
    ...
    The user already confirmed "Update and Restart" in
    AddonUpdaterInstallPopup before this runs, so a successful install
    quits Blender immediately -- no second manual "Restart blender" popup
    step anymore. An error still surfaces the existing report/manual-install
    popup, since there's nothing to restart into.
    ...
    if res is None:
        updater.print_verbose(
            "{} updater: install succeeded, quitting to restart".format(updater.addon))
