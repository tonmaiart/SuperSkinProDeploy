...
imports from core/ sub-modules -- except the shared HUD stack
(``CoreFacade.request_hud_slot``/``release_hud_slot``, see
``docs/core-interfaces/facade-api.md``), used instead of this module owning its own
draw handler, so it can never overlap another domain's line (e.g.
bone_picker's yellow "BONE PICKER") at the same screen position. Always
shows one of two lines while a mesh is in Edit Mode -- "Multi Color" while
this preview is active, "Single Color" while it's off (both currently
bone_picker's own yellow, per a later request to make every HUD line read
as one consistent color) -- rather than only appearing while active and
staying silent otherwise (see ``_sync_hud()`` below; mirrors
``features/deform_layer_viewer/deform_bone_viewer/draw.py``'s Mask/Bone HUD for the same
reason).
"""
...
_HUD_SLOT = 1  # second row from the anchor -- see docs/core-interfaces/facade-api.md's reserved-slot table
...
def _read_active_layer(obj) -> dict:
    ...
    In Edit Mode reads from __ssp_L{active_idx}_* temp VGs via bmesh --
    each Layer visited this session has its own namespaced VG set (see
    core/layer_storage/temp_vg_bridge.py), so this must read the CURRENTLY
    ACTIVE Layer's own set specifically, not just any __ssp_-prefixed VG.
    ...
    if obj.mode == 'EDIT':
        try:
            ...
            if not deform_layer:
                return {}
            active_idx = int(obj.get("__ssp_meta_layer", 0))
            ...
            for bone_id, bone_name in id_to_bone.items():
                temp_name = f"__ssp_L{active_idx}_{bone_id}"
...
def _recompute_and_write(obj, force: bool = False) -> None:
    ...
    from ...core.facade import CoreFacade as _AdhocFacade
    _AdhocFacade.debug_log(
        "adhoc:mirror_multicolor",
        f"_recompute_and_write(): obj={obj.name!r} force={force} "
        f"deform_gen={obj.get('__ssp_deform_gen', 0)} base_key={base_key} "
        f"key_changed={key != _color_key} will_recompute={force or key != _color_key}",
    )
...
def _sync_hud(obj):
    """Keep the shared HUD stack in sync with this preview's on/off state,
    reading the module-level ``_active`` flag (must already reflect the
    state being synced -- callers flip it before calling this). Pink
    "Multi Color" while active; neutral "Single Color" while inactive but
    *obj* is still a mesh in Edit Mode; no line otherwise. No-ops when the
    resolved state hasn't changed, so calling this every watcher tick
    (~10Hz) is cheap."""
...
def _watcher_tick():
    ...
    # Catches the state transitions _start_handlers()/_stop_handlers() above
    # don't already cover directly -- e.g. entering Edit Mode while the
    # preview was never toggled on (no _start_handlers() call at all), or
    # leaving Edit Mode entirely while it was already inactive (no
    # _stop_handlers() call either) -- both still need the "Single Color"
    # line requested/released. Cheap: _sync_hud() no-ops when unchanged.
...
def cleanup():
    ...
    _suppressed = False
    _stop_handlers()
    # _stop_handlers() no-ops if _active was already False -- e.g. a
    # "Single Color" line left on the HUD stack while this preview was
    # inactive. Release unconditionally so addon disable/hot-reload never
