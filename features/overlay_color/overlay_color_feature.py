...
Owns:
  ...
  - Multi Color Mask Preview (`multi_color_mask_draw.py`), the Edit Mask
    sub-mode counterpart (Edit Mode only as of 2026-09-11 -- Object/Pose
    Mode support was dropped entirely, see that module's docstring) --
    blends each vertex's color across every visible Layer, weighted by
    that Layer's mask value there, with the active/hovered Layer
    highlighted white. No keymap of its own, and -- since the
    `features/layer_picker` domain (whose Alt+1 modal used to
    auto-activate it via `bpy.ops.superskin.start_multi_color_mask()`/
    `stop_multi_color_mask()`, as did `bone_picker`'s Alt+2-while-masking
    redirect into it) was removed entirely per a later explicit user
    request -- no automatic caller either (see `keymap.py` and
    docs/domains/overlay_color.md).
  - Action dispatch: `start_multi_color` / `stop_multi_color` /
    `toggle_multi_color` (multi_color_draw.py); `start_multi_color_mask` /
    `stop_multi_color_mask` / `toggle_multi_color_mask`
    (multi_color_mask_draw.py) -- the mask trio is now reachable only via
    manual `bpy.ops` calls or the Python console, no keymap and no
    automatic caller.
"""
...
class OverlayColorFeature(UnifiedFeatureExtension):
    ...
    keymaps = [
        {
            "key": "Alt+3", "label": "Multi Color Preview (Edit Mode)", "mode": "Toggle",
            "source_label": "Multi Color Preview",
        },
        # Multi Color Mask Preview (Edit Mode only, see
        # multi_color_mask_draw.py's docstring) has no keymap entry of its
        # own, and no automatic trigger either -- it used to auto-activate
        # alongside the layer_picker domain's Alt+1, which has since been
        # removed entirely per explicit user request.
    ]
    ...
    def get_keymap_items(self) -> list:
        """Expose Alt+3's ``(km, kmi, label)`` to the in-panel shortcut
