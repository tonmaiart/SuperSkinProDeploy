...
class SUPERSKIN_PT_settings_popup(bpy.types.Panel):
    ...
    (see ``docs/domains/activate.md``), reachable without opening this
    ...
    # Default popover width (unset) was cramped once the Shortcuts section
    # (interface/utils/keymap_editor.py) added rows with a label + a
    # full_event rebind button + an optional restore icon -- widened same
    # way Blender's own USERPREF_PT_extensions_repos does it
    # (bl_ui_units_x), not via any per-row layout hack.
    bl_ui_units_x = 10
