...
class UnifiedFeatureExtension(ABC):
    ...
    def get_keymap_items(self) -> list:
        """Return this domain's registered keymap items for the in-panel
        shortcut editor (``interface/widget_preferences.py``, drawn via
        ``interface/utils/keymap_editor.py``), as ``(km, kmi, label)``
        triples: ``km``/``kmi`` are the addon-keyconfig objects the
        domain's own ``keymap.py`` registered at startup (used only to
        resolve the matching, live ``wm.keyconfigs.user`` item via
        ``interface/utils/keymap_editor.py``'s ``_resolve_user_kmi()`` --
        idname + creation-rank, NOT ``.id`` equality -- see that module's
        docstring for why addon-side ``.id`` is not stable across a
        Blender restart; rebinding must target the user keyconfig, not
        this one), ``label`` a short human-readable description shown
        next to the rebind widget.

        Default: no keymap items. A domain with a ``keymap.py`` overrides
        this to return ``keymap.get_registered_keymap_items()``.
        """
        return []
...
class UnifiedRegistry:
    ...
    def draw_settings_toggle_row(cls, layout, context) -> None:
        """Draw the "Settings"/"How to use" toggle-button row (plus their
        expanded bodies) wherever *layout* currently is.

        The sanctioned entry point for a feature domain that wants to
        place this row somewhere other than its default position --
        ``interface.widget_preferences`` itself is off-limits to
        ``features/*`` per the Interface closed-subsystem rule (see
        ``docs/core-interfaces/interface.md``'s Invariants), so this thin
        wrapper is the only way to reach it from outside ``interface/``.
        Lazily imports both ``core.facade.CoreFacade`` (to resolve
        *activated*) and ``interface.widget_preferences`` (interface-
        internal, not a ``features/*`` → ``widget_preferences`` import) to
        avoid a module-level import cycle -- ``register_api.py`` is
        imported by ``panel_main.py``, which ``widget_preferences.py`` also
        touches indirectly.

        See ``interface.widget_preferences.draw_settings_toggle_row()``
        for the actual drawing logic and its own history
        (2026-09-14, per explicit user request moving this row out of
        ``panel_main.py``'s fixed top-of-panel position into
        ``features/deform_layer_viewer/deform_layer_viewer_feature.py``'s
        own mesh-dropdown row).
        """
    ...
    def draw_collapsible_section(cls, layout, context, extension: "UnifiedFeatureExtension", tab_key: str, draw_body=None) -> None:
        """Draw *extension*'s section under *tab_key* using the same shared
        chrome every collapsible section in this addon has always used —
        plain locked-expanded label vs. a real collapsible
        ``layout.panel()``, section header, link button.

        This is the sanctioned entry point for the ``object_tools_ui`` /
        ``skin_tools_ui`` feature domains (see ``docs/domains/object_tools_ui.md``
        / ``docs/domains/skin_tools_ui.md``), which own the LAYER/SKINNING
        top-level dispatch loop themselves — iterating
        ``UnifiedRegistry.get_by_tab(tab_key)`` and calling this method per
        extension — instead of that loop living in
        ``interface.widget_preferences`` as it used to. This wrapper reuses
        the exact same chrome-drawing code (``draw_collapsible_box_ext()``)
        that the PREFERENCE-tab body still calls directly (it lives inside
        ``interface/``, so it isn't bound by the ``features/*`` import
        restriction this wrapper exists for) — nothing about how a section
        renders changed, only which caller reaches it.

        *draw_body*, a ``(layout, context) -> None`` callable, supplies the
        section's actual body content -- since a fully-migrated domain's own
        ``draw_section_for_tab()`` is now a stub, ``object_tools_ui`` /
        ``skin_tools_ui`` pass their own moved-in widget-drawing function
        here instead (e.g. ``features/skin_tools_ui/ui_mirror.py``'s
        ``draw_section()``). Omitted (the default), this falls back to
        calling *extension*'s own ``draw_section_for_tab()`` -- the original
        behavior, for any domain not yet migrated.

        ``interface.widget_preferences`` itself stays off-limits to
        ``features/*`` per the Interface closed-subsystem rule, so this thin
        wrapper is the only way to reach it from outside ``interface/`` —
        same lazy-import pattern as ``draw_settings_toggle_row()`` above, for
        the same reason (avoiding a module-level import cycle between this
        file and ``interface.widget_preferences``).
        """
