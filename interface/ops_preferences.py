...
class SUPERSKIN_PT_settings_popup(bpy.types.Panel):
    """Popover content for the top row's "Settings" button (2026-09-14, per
    explicit user request reverting it from an inline-expanding toggle back
    to a popup) -- just ``widget_preferences.draw_preferences_body()``,
    unchanged, drawn into a ``HEADER``-region popover instead of inline
    below the toggle row. See ``SUPERSKIN_PT_shortcuts_editor`` above for
    the identical ``wm.call_panel`` trigger pattern this reuses (a plain
    operator button rather than ``layout.popover()``, so no dropdown-arrow
    decoration is drawn next to the gear/lock icon).

    ``draw_preferences_body`` is imported lazily inside ``draw()`` rather
    than at module scope -- ``interface/__init__.py`` registers
    ``ops_preferences`` before ``widget_preferences`` (see
    ``docs/core-interfaces/interface.md``'s "Registration Order"), so a
    top-level import here would run before that module exists yet; by the
    time this Panel's ``draw()`` actually runs (real UI draw time, well
    after both modules finish registering), the import resolves fine.
    """
