...
A single ``bpy.utils.previews`` collection (currently eleven icons: the
...
glyphs, the bone-weight glyph, and the deform bone list's two clipboard
drop-down glyphs), loaded once at addon-register time and torn down at
unregister. No core/ or core_subsystems/ imports (only ``os``, ``gpu``,
and Blender's own ``bpy.utils.previews``), so this loads at module scope
from ``interface/utils/__init__.py`` exactly like ``gpu_utils`` does --
no deferred-loading dance needed.
...
def register():
    ...
    if os.path.isfile(_ICON_CLIPBOARD_PATH):
        _preview_collection.load("icon_clipboard", _ICON_CLIPBOARD_PATH, 'IMAGE')
    if os.path.isfile(_ICON_CLIPBOARD_FILL_PATH):
        _preview_collection.load("icon_clipboard_fill", _ICON_CLIPBOARD_FILL_PATH, 'IMAGE')
