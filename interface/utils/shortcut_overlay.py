...
one section per *category* (``interface/utils/keymap_editor.py``'s
``DOMAIN_CATEGORIES``/``category_for()`` -- "Vertex Selection", "Edit
Weight", "Display", "Misc" -- not per owning feature domain, so unrelated
domains that share a gesture family land under one header; shared with
``SUPERSKIN_PT_shortcuts_editor``'s (``interface/ops_preferences.py``)
grouping of the click-to-rebind list, one grouping definition for both),
stacked vertically and anchored to the left-middle of the 3D viewport
(offset from the edge to clear Blender's native Toolbar). Visible only
while SuperSkinPro's own "Edit Layer Weight" mode is active. Off by
...
Alt+1 to Alt+4 originally to free up Alt+1 for `deform_bone_viewer`'s
``superskin.toggle_mask_mode`` shortcut; that shortcut was itself later
removed 2026-09-10 and Alt+1 reassigned to the `layer_picker` domain --
since removed entirely, leaving Alt+1 unclaimed -- but this HUD toggle
stayed on Alt+4.)
...
line up regardless of which category they belong to. An entry whose
``"source_label"`` names a real registered shortcut (matching the label
its domain's ``keymap.py::get_registered_keymap_items()`` used) has its
displayed key text resolved live from the actual current
``wm.keyconfigs.user`` binding (``_row_key_text()`` below, via
``keymap_editor.resolve_live_key_text()``) instead of showing a
hand-typed literal that could drift out of sync after a rebind made
through ``SUPERSKIN_PT_shortcuts_editor``. An entry with no
``source_label`` (e.g. bone_picker's ``sub_keymaps``, which describe
in-modal mouse actions that have no separate ``KeyMapItem`` of their
own) keeps its plain hand-typed ``"key"`` text, since there is nothing
live to resolve it against.
...
# several unrelated domains' keymaps land under the same header. The
# category table itself lives in interface/utils/keymap_editor.py
# (DOMAIN_CATEGORIES/CATEGORY_ORDER/category_for) -- shared with
# SUPERSKIN_PT_shortcuts_editor's (interface/ops_preferences.py) grouping
# of the click-to-rebind list, so this HUD and that popover always agree on
# which category a domain's shortcuts fall under.


def _row_key_text(context, km: dict) -> str:
    """The key text to draw for one ``get_keymaps()`` entry -- resolved
    live from the actual, possibly-rebound ``wm.keyconfigs.user`` item
    when the entry names a ``source_label`` (matching the ``label`` that
    domain's ``keymap.py::get_registered_keymap_items()`` registered it
    under), falling back to the entry's own hand-typed ``"key"`` text
    otherwise -- either because it has none (e.g. bone_picker's
    sub_keymaps describe in-modal mouse actions with no separate
    KeyMapItem of their own) or because live resolution failed (e.g.
    mid-reload). This is what keeps this HUD's displayed key in sync
    with a rebind made through ``SUPERSKIN_PT_shortcuts_editor`` instead
    of silently showing a stale default forever."""
...
def _collect_domains(context):
    ...
    # CATEGORY_ORDER -- any category not in that tuple (from an unmapped
