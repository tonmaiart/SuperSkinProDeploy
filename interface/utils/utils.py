...
def _run_preserving_edit_session(context, callback, *args):
    """Like ``_run_in_object_context``, but does NOT force a real
    EDIT->OBJECT->EDIT mode bounce when the mesh is already in EDIT mode --
    it calls *callback(*args)* directly instead.

    ``core/ui_controller/layer_crud.py``'s structural Layer CRUD functions
    (``create_layer``, ``remove_layer``, ``move_layer``, ``duplicate_layer``,
    ``merge_selected_layers``, ``toggle_visible``, ``set_layer_icon``) are
    Edit-Mode-session-aware: they read ``ctrl.obj.mode`` themselves to decide
    whether to route through the in-session ``__ssp_meta_list`` pending
    mirror / per-Layer temp VGs instead of permanent storage (see
    ``core/layer_storage/temp_vg_bridge.py``'s ``effective_meta_list()``),
    and perform their own tightly-scoped internal OBJECT-mode bounce only
    for the specific moments that genuinely require it (temp-VG create/
    delete).

    Wrapping one of those calls in ``_run_in_object_context`` instead makes
    the real ``obj.mode`` read ``'OBJECT'`` for the whole call, so those
    internal ``ctrl.obj.mode == 'EDIT'`` checks never fire -- the CRUD
    silently writes straight to permanent storage while temp VGs are still
    resident, and ``sync_layers_to_ui_collection()`` (which runs afterward,
    once the mesh is back in EDIT mode) then reads the untouched, stale
    pending mirror -- so the Layer list appears not to reflect the change at
    all until a real Exit Edit Mode. Use this helper instead for any call
    into one of the functions above.
    """
    obj = context.active_object
    if obj.mode == 'EDIT':
...
def sync_bones_to_ui_collection(obj):
    ...
    try:
        ...
        from ...core.layer_storage.temp_vg_bridge import effective_meta_list
        _sto = LayerStorageService(obj.data)
        _meta = effective_meta_list(obj, _sto)
...
def sync_layers_to_ui_collection(obj):
    ...
    While an Edit Mode session is active, structural layer CRUD
    (Add/Remove/Duplicate/Merge/Move/Rename/Visibility) writes to the
    in-session pending mirror (__ssp_meta_list) instead of the permanent
    ss_layers_meta -- see core/ui_controller/layer_crud.py's
    flush_edit_session() -- so this prefers that pending mirror whenever
    one exists, falling back to permanent storage otherwise. Same for the
    active-layer pointer (__ssp_meta_layer vs. ss_active_layer).
    """
    from ...core.layer_storage.temp_vg_bridge import (
        has_temp_vgs, read_pending_meta_list, get_active_layer_from_meta,
    )

    in_session = obj.mode == 'EDIT' and has_temp_vgs(obj)
    meta = read_pending_meta_list(obj) if in_session else None
    meta_raw = None
    ...
    col = obj.superskin_layers_collection
    active_idx = get_active_layer_from_meta(obj) if in_session else obj.data.get("ss_active_layer", 0)
    ...
    # Dedupe cache keyed on the raw permanent-storage string specifically
    # (used by the depsgraph handler below to skip redundant resyncs) --
    # only updated when this call actually read permanent storage; a
    # pending-mirror-sourced sync doesn't change what that string is.
