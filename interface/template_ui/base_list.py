...
class SuperSkinListMixin:
    ...
    def draw_main_icon(self, context, data, item) -> str:
        ...
        Layers: the layer's own user-assigned ``item.icon`` (default
        ``'NONE'``) -- see ``features/deform_layer_viewer/layer_viewer/ops.py``'s
        ``SUPERSKIN_OT_layer_icon_picker``.
        """
        raise NotImplementedError("draw_main_icon()")

    def draw_main_icon_value(self, context, data, item) -> int:
        """Return a custom ``icon_value`` for the row's leading icon,
        overriding the built-in ``draw_main_icon()`` string enum for this
        one row -- ``0`` (default, Blender's "no icon" sentinel) means "no
        custom icon here, use ``draw_main_icon()``'s string instead."

        Only the Layer list currently overrides this (its per-row
        mask-state/color icon, sourced from ``assets/layer_icons/`` via
        ``interface/utils/icons.py``'s ``get_layer_state_icon_id()`` --
        see ``SUPERSKIN_UL_layer_list_view`` in
        ``features/deform_layer_viewer/layer_viewer/ui.py``). Every other
        list (e.g. Bones) is unaffected by this hook's existence.
        """
        return 0
