...
row in the top row itself (see ``docs/domains/activate.md``), not here.
...
def _draw_skinning_interface(layout, context, prefs):
    ...
    click -- see `docs/domains/layer_viewer.md`) -- this function used to
...
def _draw_preferences(layout, context, prefs):
    ...
    ``docs/domains/activate.md``), reachable without opening this
    ...
    # plain operator button doesn't get. Same fix as the top row's gear
    # button, see panel_main.py's _draw_top_row() docstring (item 2).
